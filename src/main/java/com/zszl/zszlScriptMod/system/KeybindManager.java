...
public class KeybindManager {
    ...
    public static void executeAction(BindableAction action) {
        ...
        switch (action) {
            ...
            case RESET_WINDOW_LAYOUT:
                ...
                break;
            case TOGGLE_TAB_FULLSCREEN:
                if (mc.currentScreen instanceof GuiModernMainScreen && !DetachedSwingWindowManager.isDetached()) {
                    activeScreen = activeModernScreen(mc);
                    if (activeScreen != null) {
                        activeScreen.toggleActiveTabFullscreen();
                    }
                }
                break;
    ...
    public static String menuShortcut(BindableAction action) {
        if (action == null) {
            return "";
        }
        Keybind keybind = keybinds.get(action);
        ...
        String formatted = keybind.toString();
        if (formatted == null || formatted.isEmpty()) {
            return "";
        }
        String unbound = I18n.format("gui.keybind.unbound");
        if (formatted.equals(unbound)) {
            return "";
        }
        int slash = formatted.indexOf(" / ");
        if (slash >= 0) {
            formatted = formatted.substring(0, slash);
        }
    ...
    private static void ensureDefaultKeybinds() {
        ...
        putDefault(BindableAction.TOGGLE_TAB_FULLSCREEN, Keyboard.KEY_M, Keyboard.KEY_LCONTROL, Keyboard.KEY_LSHIFT);
        ...
        putDefault(BindableAction.CLOSE_ALL_TABS, Keyboard.KEY_W, Keyboard.KEY_LCONTROL, Keyboard.KEY_LSHIFT);
