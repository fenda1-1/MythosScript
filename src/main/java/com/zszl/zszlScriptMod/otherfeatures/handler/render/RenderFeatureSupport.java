...
final class RenderFeatureSupport {
    ...
    private static final float ENTITY_TAG_LABEL_BASE_SCALE = 0.012F;
    private static final double ENTITY_TAG_LABEL_REFERENCE_DISTANCE = 10.0D;
    ...
    static void renderEntityInfo(Minecraft mc) {
        ...
        Entity entity = findEntityUnderCrosshair(mc, RenderFeatureManager.getEntityInfoMaxDistance());
    ...
    private static Entity findEntityUnderCrosshair(Minecraft mc, double maxDistance) {
        Entity viewer = mc.getRenderViewEntity();
        if (viewer == null || mc.world == null) {
            return null;
        }

        double range = Math.max(RenderFeatureManager.ENTITY_DISTANCE_MIN,
                Math.min(RenderFeatureManager.ENTITY_DISTANCE_MAX, maxDistance));
        Vec3d eyePos = viewer.getPositionEyes(1.0F);
        Vec3d lookVec = viewer.getLook(1.0F);
        Vec3d endPos = eyePos.addVector(lookVec.x * range, lookVec.y * range, lookVec.z * range);
        RayTraceResult blockHit = mc.world.rayTraceBlocks(eyePos, endPos, false, true, false);
        double blockDistanceSq = blockHit != null && blockHit.hitVec != null
                ? eyePos.squareDistanceTo(blockHit.hitVec)
                : Double.POSITIVE_INFINITY;
        AxisAlignedBB searchBox = new AxisAlignedBB(
                Math.min(eyePos.x, endPos.x), Math.min(eyePos.y, endPos.y), Math.min(eyePos.z, endPos.z),
                Math.max(eyePos.x, endPos.x), Math.max(eyePos.y, endPos.y), Math.max(eyePos.z, endPos.z))
                .grow(1.0D);

        Entity closestEntity = null;
        double closestDistanceSq = blockDistanceSq;
        for (Entity candidate : mc.world.getEntitiesWithinAABBExcludingEntity(viewer, searchBox)) {
            if (candidate == null || candidate.isDead || !candidate.canBeCollidedWith()) {
                continue;
            }
            AxisAlignedBB boundingBox = candidate.getEntityBoundingBox();
            if (boundingBox == null) {
                continue;
            }
            double border = Math.max(0.0D, candidate.getCollisionBorderSize());
            ...
            double distanceSq = eyePos.squareDistanceTo(hitVec);
    ...
    private static boolean containsPoint(AxisAlignedBB box, Vec3d point) {
        return box != null && point != null
    ...
    private static double getMaxLivingDistanceSq() {
        ...
        if (RenderFeatureManager.isEnabled("entity_tags")) {
            maxDistance = Math.max(maxDistance, RenderFeatureManager.getEntityTagMaxDistance());
        }
    ...
    private static void renderItemLabel(Minecraft mc, EntityItem entityItem, float partialTicks) {
        ...
        drawWorldLabel(mc, getEntityCenter(entityItem, partialTicks).addVector(0.0D, 0.35D, 0.0D),
                lines, 0xFFFFFFAA, RenderFeatureManager.itemEspThroughWalls, false);
    }

    private static void drawWorldLabel(Minecraft mc, Vec3d worldPos, List<String> lines, int textColor,
            boolean throughWalls, boolean fixedScreenSize) {
        ...
        double scaleDistance = Math.max(1.0D, cameraDistance);
        if (fixedScreenSize) {
            // Keep nearby labels readable while preventing distant labels from
            // growing into large screen-filling panels. Ten blocks is the
            // reference distance used by the entity-tag feature by default.
            scaleDistance = Math.min(scaleDistance, ENTITY_TAG_LABEL_REFERENCE_DISTANCE);
        }
        float scale = (fixedScreenSize ? ENTITY_TAG_LABEL_BASE_SCALE : 0.026F)
                * (fixedScreenSize ? (float) scaleDistance : 1.0F);
    ...
    private static void renderEntityTags(Minecraft mc, float partialTicks, RenderFrameData frameData) {
        double maxDistance = RenderFeatureManager.getEntityTagMaxDistance();
        double maxDistanceSq = maxDistance * maxDistance;
        for (EntityRenderSample sample : frameData.livingEntities) {
