...
public final class ActionTargetLocator {
    ...
    public static boolean tryInvokeCurrentScreenClick(int x, int y, String mouseButton) {
        ...
        try {
            Method mouseClicked = resolveMethod(screen.getClass(), new String[] { "mouseClicked", "func_73864_a" },
                    int.class, int.class, int.class);
            mouseClicked.invoke(screen, x, y, mouseButtonCode);
            Method mouseReleased = resolveMethod(screen.getClass(), new String[] { "mouseReleased", "func_146286_b" },
                    int.class, int.class, int.class);
            ...
            zszlScriptMod.LOGGER.warn("GUI semantic click injection failed: screen={}, point=({}, {}), button={}",
                    screen.getClass().getName(), x, y, mouseButton, cause);
        }
        return false;
    }

    /** Injects a keyboard event into the active GUI, including a detached modern screen. */
    public static boolean tryInvokeCurrentScreenKey(char typedChar, int keyCode) {
        GuiScreen screen = GuiElementInspector.getActiveScreen();
        if (screen == null) {
            return false;
        }
        try {
            Method keyTyped = resolveMethod(screen.getClass(), new String[] { "keyTyped", "func_73869_a" },
                    char.class, int.class);
            keyTyped.invoke(screen, typedChar, keyCode);
            return true;
        } catch (Exception error) {
            Throwable cause = error instanceof InvocationTargetException
                    && ((InvocationTargetException) error).getCause() != null
                            ? ((InvocationTargetException) error).getCause() : error;
            zszlScriptMod.LOGGER.warn("GUI semantic key injection failed: screen={}, keyCode={}",
                    screen.getClass().getName(), keyCode, cause);
            return false;
        }
    }
    ...
    private static Method resolveMethod(Class<?> type, String name, Class<?>... parameterTypes)
        ...
        return resolveMethod(type, new String[] { name }, parameterTypes);
    }

    private static Method resolveMethod(Class<?> type, String[] names, Class<?>... parameterTypes)
            throws NoSuchMethodException {
        Class<?> current = type;
        while (current != null) {
            for (String name : names) {
