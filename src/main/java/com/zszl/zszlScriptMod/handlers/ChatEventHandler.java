...
public class ChatEventHandler {
    ...
    public void onMcpReceivedChat(net.minecraftforge.client.event.ClientChatReceivedEvent event) {
        com.zszl.zszlScriptMod.mcp.McpChatCapture.capture("received", event.getType().name(), event.getMessage(), 0);
    }

    @SubscribeEvent(priority = EventPriority.HIGHEST)
    public void onMcpConnect(net.minecraftforge.fml.common.network.FMLNetworkEvent.ClientConnectedToServerEvent event) {
        com.zszl.zszlScriptMod.mcp.McpChatHistory.INSTANCE.connect(event.getManager(),
                String.valueOf(event.getManager().getRemoteAddress()));
    }

    @SubscribeEvent(priority = EventPriority.HIGHEST)
    public void onMcpDisconnect(net.minecraftforge.fml.common.network.FMLNetworkEvent.ClientDisconnectionFromServerEvent event) {
        com.zszl.zszlScriptMod.mcp.McpChatHistory.INSTANCE.disconnect(event.getManager());
    }
    ...
    public static void triggerDisplayedChatMessage(ITextComponent originalComponent, ITextComponent displayedComponent,
        ...
        // A number of mature handlers still emit their debug line directly
        // to the client chat instead of through ModConfig.debugPrint(). Feed
        // those legacy lines into the bounded terminal buffer as well.
        DebugLogManager.appendChatDebug(displayedMessage.isEmpty() ? rawMessage : displayedMessage);
        com.zszl.zszlScriptMod.mcp.McpChatCapture.capture("displayed", "DISPLAYED", displayedComponent, chatLineId);
