...
import com.zszl.zszlScriptMod.gui.modern.ModernPageStateRecorder;
import com.zszl.zszlScriptMod.gui.modern.ModernSplitPane;
import com.zszl.zszlScriptMod.gui.modern.ModernTabCatalog;
import com.zszl.zszlScriptMod.gui.modern.ModernTabOrder;
import com.zszl.zszlScriptMod.gui.modern.ModernTabStripLayout;
 ...
 */
public class GuiModernMainScreen extends GuiScreen
        implements ModernTooltipSupport.OwnsTooltipAnchors, GuiElementInspector.McpElementProvider {

    private static final int TAB_STRIP_HEIGHT = ModernTabStripLayout.HORIZONTAL_STRIP_HEIGHT;
    ...
    private static final long PAGE_STATE_CAPTURE_INTERVAL_NANOS = 180_000_000L;
    private static final int[] DASHBOARD_CARD_WIDTH_OPTIONS = { 0, 140, 180, 220, 260, 300, 360 };
    private static final int[] DASHBOARD_CARD_HEIGHT_OPTIONS = { 0, 40, 56, 72, 96, 120, 144 };
    private static final int[] DASHBOARD_TEXT_SCALE_OPTIONS = { 75, 90, 100, 110, 125, 150, 175, 200 };
    private static final int[] DASHBOARD_SETTINGS_WIDTH_OPTIONS = { 180, 220, 260, 320, 400, 480 };
    ...
    private static final TargetType[] COLLAPSED_HEADER_ACTION_TYPES = {
            TargetType.HEADER_SEARCH,
            TargetType.COMMAND_PALETTE,
            TargetType.GLOBAL_SETTINGS,
            TargetType.STOP_FOREGROUND,
            TargetType.STOP_BACKGROUND,
            TargetType.PATHS,
            TargetType.TOOLS,
            TargetType.DETACH,
            TargetType.CLOSE
    };
    private static final int[] COLLAPSED_HEADER_ACTION_ICONS = { 8, 7, 6, 0, 1, 2, 3, 4, 5 };

    private enum CloseConfirmKind {
        TAB,
        ALL,
        OTHERS
    }

    private enum ConfirmButtonStyle {
        DANGER,
        ACCENT,
        NEUTRAL
    }
    ...
    private final Map<String, JsonObject> pendingPageStates = new LinkedHashMap<>();
    private final Map<String, String> pageStateSignatures = new HashMap<>();
    private final Map<String, Long> pageStateCaptureTimes = new HashMap<>();
    private final Set<ModernSettingsTab> restorePageStateAfterFirstDraw = Collections
            .newSetFromMap(new IdentityHashMap<ModernSettingsTab, Boolean>());
    private final Map<ModernSettingsTab, String> pendingTabFocusAfterStateRestore = new IdentityHashMap<>();
    ...
    private ModernTabStripLayout.Layout lastTabStrip;
    ...
    private ModernMainLayout.Rect tabStripDividerBounds;
    ...
    private final ModernHoverScrollbar horizontalTabScrollbar = new ModernHoverScrollbar(ModernHoverScrollbar.Axis.HORIZONTAL);
    private final ModernHoverScrollbar verticalTabScrollbar = new ModernHoverScrollbar(ModernHoverScrollbar.Axis.VERTICAL);
    private ModernHoverScrollbar tabScrollbar = horizontalTabScrollbar;
    private final ModernHoverScrollbar dashboardScrollbar = new ModernHoverScrollbar();
    private final ModernHoverScrollbar dashboardSettingsScrollbar = new ModernHoverScrollbar();
    ...
    private ModernMainLayout.Rect dashboardSettingsViewportBounds;
    private ModernMainLayout.Rect dashboardSettingsResizeBounds;
    ...
    private int dashboardSettingsScroll;
    private int dashboardSettingsMaxScroll;
    private boolean draggingDashboardSettingsResize;
    private int dashboardSettingsResizeStartMouseX;
    private int dashboardSettingsResizeStartWidth;
    private int dashboardSettingsResizePreviewWidth;
    ...
    private CloseConfirmKind closeConfirmationKind = CloseConfirmKind.TAB;
    ...
    private ModernMainLayout.Rect closeConfirmationSaveBounds;
    ...
    private int tabStripInnerWidth = -1;
    ...
    private TabKey fullscreenTab;
    private boolean draggingSidebar;
    private boolean draggingTabStrip;
    private boolean draggingTabStripHeight;
    private boolean draggingNavigationScrollbar;
    private boolean draggingTabScrollbar;
    private boolean tabReorderPending;
    private boolean tabReorderActive;
    private TabKey tabReorderKey;
    private int tabReorderStartX;
    private int tabReorderStartY;
    private int tabReorderDropInsertBefore;
    private final Map<TabKey, ModernMainLayout.Rect> lastRenderedTabBounds = new HashMap<>();
    private int sidebarDragStartWidth;
    private int tabStripDragStartWidth;
    private int tabStripDragStartHeight;
    private int tabStripDragStartMouseY;
    private long lastSidebarDividerClickAt;
    private int lastSidebarDividerClickX;
    private long lastTabStripDividerClickAt;
    private int lastTabStripDividerClickX;
    private long lastTabStripHeightDividerClickAt;
    private int lastTabStripHeightDividerClickY;
    ...
    /**
     * Opens a settings route from MCP without requiring the caller to know
     * which dashboard card or tab-strip coordinate currently contains it.
     */
    public boolean openSettingsTabFromMcp(String command) {
        String normalized = resolveMcpRouteCommand(normalizeMcpRoute(command));
        if (!isMcpOpenableCommand(normalized)) {
            return false;
        }
        closeCommandPalette();
        ...
        openSettingsTabForCommand(normalized);
        return !activeTab.isDashboard() && normalized.equals(commandForTabKey(activeTab));
    }

    private boolean isMcpOpenableCommand(String command) {
        if (command == null || command.isEmpty()) {
            return false;
        }
        return "action_editor".equals(command) || "expression_editor".equals(command)
                || "action_variables".equals(command) || isPacketChildCommand(command)
                || tabKeyForCommand(command) != null;
    }

    /** Selects an already-open tab by route, encoded tab id or one-based slot. */
    public boolean selectTabFromMcp(String target) {
        String value = normalizeMcpTabTarget(target);
        TabKey key = null;
        if (value.isEmpty() || "dashboard".equalsIgnoreCase(value)) {
            key = TabKey.dashboard();
        } else if (value.startsWith("fixed:") || value.startsWith("cmd:")) {
            try {
                key = TabKey.decode(value);
            ...
            try {
                ...
                if (slot > 0 && slot <= openTabs.size()) {
                    key = openTabs.get(slot - 1);
                }
            } catch (NumberFormatException ignored) {
                key = tabKeyForCommand(value);
            }
        }
        if (key == null) {
            key = findTabKeyByMcpSegment(value);
        }
        ...
        closeCommandPalette();
        toolsOpen = false;
        if (!key.isDashboard() && searchField != null) {
            searchField.setFocused(false);
        }
        return activeTab.equals(key);
    }

    /** Closes the modern screen, the active tab, or all closable tabs. */
    public boolean closeFromMcp(String scope) {
        String value = scope == null || scope.trim().isEmpty() ? "screen" : scope.trim().toLowerCase(Locale.ROOT);
        closeCommandPalette();
        if ("tab".equals(value) || "active_tab".equals(value)) {
        ...
        if ("all_tabs".equals(value) || "tabs".equals(value)) {
            requestCloseAllTabs();
            return true;
        }
        if ("screen".equals(value) || "menu".equals(value)) {
            requestCloseScreen();
            return true;
        }
        throw new IllegalArgumentException("Unknown GUI close scope: " + scope);
    }

    /**
     * Updates a text input owned by the modern shell or the active settings
     * tab. This mirrors a user typing into the field and leaves persistence to
     * the tab's normal Save action.
     */
    public boolean setMcpText(String target, String text, boolean append) {
        ...
        if (matchesMcpTarget(value, "search") && searchField != null) {
            searchField.setText(append ? searchField.getText() + incoming : incoming);
            searchField.setFocused(true);
            focusDashboardForSearch();
            return true;
        }
        if (matchesMcpTarget(value, "command_palette_search") && commandPaletteSearchField != null) {
            commandPaletteSearchField.setText(append ? commandPaletteSearchField.getText() + incoming : incoming);
            commandPaletteSearchField.setFocused(true);
            refreshCommandPaletteEntries();
            return true;
        }
        ModernSettingsTab tab = getActiveSettingsTab();
        return tab != null && tab.setMcpText(value, incoming, append);
    }
    ...
    public boolean setMcpValue(String target, String value) {
        ...
        if (matchesMcpTarget(normalizedTarget, "search")
                || matchesMcpTarget(normalizedTarget, "command_palette_search")) {
            return setMcpText(normalizedTarget, incoming, false);
        }
        Boolean booleanValue = parseMcpBoolean(incoming);
        if (booleanValue != null && matchesMcpTarget(normalizedTarget, "auto_focus")) {
            MainUiLayoutManager.setModernAutoFocus(booleanValue.booleanValue());
            return true;
        }
        if (booleanValue != null && matchesMcpTarget(normalizedTarget, "auto_pause")) {
            MainUiLayoutManager.setModernAutoPauseOnMenuOpen(booleanValue.booleanValue());
            return true;
        }
        if (booleanValue != null && matchesMcpTarget(normalizedTarget, "running_status")) {
            MainUiLayoutManager.setModernRunningStatusVisible(booleanValue.booleanValue());
            return true;
        }
        ModernSettingsTab tab = getActiveSettingsTab();
        return tab != null && tab.setMcpValue(normalizedTarget, incoming);
    }

    public boolean revealMcpTarget(String target) {
        ModernSettingsTab tab = getActiveSettingsTab();
        return tab != null && tab.revealMcpTarget(target);
    }

    private boolean matchesMcpTarget(String target, String name) {
    ...
    public String getMcpActiveTabId() {
        return activeTab == null ? "" : activeTab.encode();
    }

    public String getMcpActiveTabCommand() {
        return activeTab == null ? "" : stateCommandForTab(activeTab);
    }

    public List<String> getMcpOpenTabIds() {
        List<String> result = new ArrayList<>();
        for (TabKey tab : openTabs) {
            if (tab != null) {
                result.add(tab.encode());
            }
        }
        return result;
    }

    public List<String> getMcpOpenTabCommands() {
        List<String> result = new ArrayList<>();
        for (TabKey tab : openTabs) {
            if (tab != null) {
                result.add(stateCommandForTab(tab));
            }
        }
        return result;
    }

    /** Lists every route accepted by openSettingsTabFromMcp. */
    public List<String> getMcpAvailableTabCommands() {
        List<String> result = new ArrayList<>();
        result.addAll(tabRegistry.commands());
        result.add("action_editor");
        result.add("expression_editor");
        result.add("action_variables");
        return result;
    }

    public boolean isMcpCommandPaletteOpen() {
        return commandPaletteOpen;
    }

    /**
     * Rebuilds the layout and hit map immediately after an MCP mutation. The
     * normal render loop will draw again, but MCP responses must describe the
     * new tab/section instead of the previous frame.
     */
    @Override
    public void prepareMcpGui() {
        if (closed || fontRenderer == null) {
            return;
        }
        try {
            drawScreen(0, 0, 0.0F);
        } catch (RuntimeException exception) {
            zszlScriptMod.LOGGER.debug("Unable to eagerly rebuild MCP GUI layout", exception);
        }
    }

    /**
     * Publishes the hit targets already used by the renderer. The returned
     * paths are stable semantic targets; their coordinates are only a
     * compatibility detail for the existing click injector.
     */
    @Override
    public List<GuiElementInspector.GuiElementInfo> getMcpGuiElements() {
        List<GuiElementInspector.GuiElementInfo> result = new ArrayList<>();
        Set<String> paths = new HashSet<>();
        for (HitTarget target : hitTargets) {
            GuiElementInspector.GuiElementInfo element = toMcpElement(target);
            if (element != null && paths.add(element.getPath())) {
                result.add(element);
            }
        }
        if (searchBounds != null && searchField != null) {
            addMcpElement(result, paths, new GuiElementInspector.GuiElementInfo(
                    GuiElementInspector.ElementType.CUSTOM,
                    "screen/GuiModernMainScreen/header/search",
                    searchField.getText(), searchBounds.x, searchBounds.y, searchBounds.width, searchBounds.height,
                    Integer.MIN_VALUE, -1, "input", searchField.getText(), true, true,
                    java.util.Arrays.asList("click", "input", "set"), java.util.Collections.<String>emptyList()));
        }
        if (commandPaletteOpen && commandPaletteSearchBounds != null && commandPaletteSearchField != null) {
            addMcpElement(result, paths, new GuiElementInspector.GuiElementInfo(
                    GuiElementInspector.ElementType.CUSTOM,
                    "screen/GuiModernMainScreen/command_palette/search",
                    commandPaletteSearchField.getText(), commandPaletteSearchBounds.x, commandPaletteSearchBounds.y,
                    commandPaletteSearchBounds.width, commandPaletteSearchBounds.height, Integer.MIN_VALUE, -1,
                    "input", commandPaletteSearchField.getText(), true, true,
                    java.util.Arrays.asList("click", "input", "set"), java.util.Collections.<String>emptyList()));
        }
        ModernSettingsTab settingsTab = getActiveSettingsTab();
        if (settingsTab != null) {
            String prefix = "screen/GuiModernMainScreen/tab/" + mcpSegment(getMcpActiveTabCommand()) + "/";
            List<GuiElementInspector.GuiElementInfo> tabElements = settingsTab.getMcpGuiElements(prefix);
            if (tabElements != null) {
                for (GuiElementInspector.GuiElementInfo element : tabElements) {
                    addMcpElement(result, paths, qualifyMcpElement(element, prefix));
                }
            }
        }
        return result;
    ...
    private void addMcpElement(List<GuiElementInspector.GuiElementInfo> result, Set<String> paths,
            GuiElementInspector.GuiElementInfo element) {
        if (element == null || element.getPath() == null || element.getPath().trim().isEmpty()
                || element.getWidth() <= 0 || element.getHeight() <= 0 || !paths.add(element.getPath())) {
            return;
        }
        result.add(element);
    }

    private GuiElementInspector.GuiElementInfo qualifyMcpElement(GuiElementInspector.GuiElementInfo element,
            String prefix) {
        if (element == null || element.getPath() == null || element.getPath().startsWith("screen/")) {
            return element;
        }
        String path = element.getPath().replace('\\', '/');
        while (path.startsWith("/")) path = path.substring(1);
        return element.withPath(prefix + path);
    }

    private GuiElementInspector.GuiElementInfo toMcpElement(HitTarget target) {
        if (target == null || target.type == null || target.bounds == null
        ...
        switch (target.type) {
        case COMMAND:
            if (target.value == null || target.value.trim().isEmpty()) return null;
            path = "screen/GuiModernMainScreen/dashboard/command/" + mcpSegment(target.value);
            text = GuiInventory.findBaseItemName(target.value);
            break;
        case TAB_SELECT:
            path = "screen/GuiModernMainScreen/tabs/select/" + mcpSegment(target.value);
            text = ModernFormI18n.tr(getTabTitle(TabKey.decode(target.value)));
            break;
        case TAB_CLOSE:
            path = "screen/GuiModernMainScreen/tabs/close/" + mcpSegment(target.value);
            text = "关闭 " + ModernFormI18n.tr(getTabTitle(TabKey.decode(target.value)));
            break;
        case CATEGORY:
            if (target.categoryRow == null) return null;
            path = "screen/GuiModernMainScreen/navigation/category/"
                    + mcpSegment(target.categoryRow.category + "/" + target.categoryRow.subCategory);
            text = getNavigationRowLabel(new NavigationRow(target.categoryRow, false));
            break;
        case CATEGORY_EXPAND:
            path = "screen/GuiModernMainScreen/navigation/category_expand/" + mcpSegment(target.value);
            text = target.value;
            break;
        case OTHER_FEATURE_GROUP:
            path = "screen/GuiModernMainScreen/navigation/other_features";
            text = I18n.format("gui.inventory.other_features");
            break;
        case NAVIGATION_TOGGLE:
            path = "screen/GuiModernMainScreen/navigation/toggle";
            text = tr("gui.modern.navigation", "导航");
            break;
        case COMMAND_PALETTE:
            path = "screen/GuiModernMainScreen/header/command_palette";
            text = tr("gui.modern.main.palette.title", "打开命令面板");
            break;
        case TOOLS:
            path = "screen/GuiModernMainScreen/header/tools";
            text = tr("gui.modern.main.u008", "工具");
            break;
        case CLOSE:
            path = "screen/GuiModernMainScreen/header/close";
            text = tr("gui.modern.main.u010", "关闭");
            break;
        case DETACH:
            path = "screen/GuiModernMainScreen/header/detach";
            text = tr("gui.modern.main.u009", "分离窗口");
            break;
        case PATHS:
            path = "screen/GuiModernMainScreen/header/paths";
            text = tr("gui.modern.main.u006", "路径");
            break;
        case GLOBAL_SETTINGS:
            path = "screen/GuiModernMainScreen/header/global_settings";
            text = tr("gui.modern.global_settings", "全局设置");
            break;
        case HEADER_SEARCH:
            path = "screen/GuiModernMainScreen/header/search";
            text = tr("gui.modern.main.u003", "搜索功能或路径");
            break;
        case DASHBOARD_SETTINGS:
            path = "screen/GuiModernMainScreen/dashboard/settings";
            text = tr("gui.modern.main.u031", "仪表盘设置");
            break;
        case DASHBOARD_GROUP:
            path = "screen/GuiModernMainScreen/dashboard/group/" + mcpSegment(target.value);
            text = target.value;
            break;
        case DASHBOARD_PAGE:
            path = "screen/GuiModernMainScreen/dashboard/page/" + mcpSegment(target.value);
            text = target.value;
            break;
        case DASHBOARD_TOOL:
            path = "screen/GuiModernMainScreen/dashboard/tool/" + mcpSegment(target.value);
            text = target.value;
            break;
        case DASHBOARD_TOOL_OPTION:
            path = "screen/GuiModernMainScreen/dashboard/tool_option/" + mcpSegment(target.value);
            text = target.value;
            break;
        case AUTO_FOCUS:
            path = "screen/GuiModernMainScreen/sidebar/auto_focus";
            text = tr("gui.modern.auto_focus_short", "自动聚焦");
            break;
        case AUTO_PAUSE:
            path = "screen/GuiModernMainScreen/dashboard/auto_pause";
            text = tr("gui.modern.auto_pause", "自动暂停");
            break;
        case RUNNING_STATUS_TOGGLE:
            path = "screen/GuiModernMainScreen/dashboard/running_status";
            text = tr("gui.modern.running_status", "运行状态");
            break;
        default:
            return null;
        }
        String controlType = target.type == TargetType.NAVIGATION_TOGGLE || target.type == TargetType.AUTO_PAUSE
                || target.type == TargetType.AUTO_FOCUS || target.type == TargetType.RUNNING_STATUS_TOGGLE
                        ? "toggle"
                        : target.type == TargetType.TAB_SELECT ? "tab"
                                : target.type == TargetType.COMMAND ? "route" : "action";
        String currentValue = "";
        if (target.type == TargetType.AUTO_FOCUS) currentValue = String.valueOf(MainUiLayoutManager.isModernAutoFocus());
        else if (target.type == TargetType.AUTO_PAUSE) currentValue = String.valueOf(MainUiLayoutManager.isModernAutoPauseOnMenuOpen());
        else if (target.type == TargetType.RUNNING_STATUS_TOGGLE) currentValue = String.valueOf(MainUiLayoutManager.isModernRunningStatusVisible());
        List<String> actions = target.type == TargetType.TAB_SELECT
                ? java.util.Arrays.asList("click", "select")
                        : "toggle".equals(controlType)
                                ? java.util.Arrays.asList("click", "set")
                                : java.util.Collections.singletonList("click");
        return new GuiElementInspector.GuiElementInfo(GuiElementInspector.ElementType.CUSTOM, path,
                text == null ? "" : text, target.bounds.x, target.bounds.y, target.bounds.width, target.bounds.height,
                Integer.MIN_VALUE, -1, controlType, currentValue, true, "toggle".equals(controlType), actions,
    ...
    /**
     * The modern renderer lays out in logical coordinates when its UI scale is
     * above 100%, while GuiScreen mouse events arrive in the scaled screen
     * coordinate space. Normalize only custom elements; vanilla buttons and
     * container slots already use the latter space.
     */
    @Override
    public GuiElementInspector.GuiElementInfo normalizeMcpGuiElement(
            GuiElementInspector.GuiElementInfo element) {
        ...
        int left = toScreenCoordinate(element.getX(), originX, scale);
        int top = toScreenCoordinate(element.getY(), originY, scale);
        int right = toScreenCoordinate(element.getX() + element.getWidth(), originX, scale);
        int bottom = toScreenCoordinate(element.getY() + element.getHeight(), originY, scale);
        return element.withGeometry(left, top, Math.max(1, right - left), Math.max(1, bottom - top));
    }

    private int toScreenCoordinate(int logicalCoordinate, int origin, float scale) {
        return Math.round((logicalCoordinate + origin) * scale);
    }
    ...
    private String normalizeMcpRoute(String value) {
        String target = value == null ? "" : value.trim().replace('\\', '/');
        String lower = target.toLowerCase(Locale.ROOT);
        int tabMarker = lower.indexOf("/tab/");
        if (tabMarker >= 0) {
            String route = target.substring(tabMarker + 5);
            int slash = route.indexOf('/');
            return (slash < 0 ? route : route.substring(0, slash)).trim();
        }
        int commandMarker = lower.indexOf("/dashboard/command/");
        if (commandMarker >= 0) {
            String route = target.substring(commandMarker + "/dashboard/command/".length());
            int slash = route.indexOf('/');
            return (slash < 0 ? route : route.substring(0, slash)).trim();
        }
        return target.startsWith("tab:") ? target.substring(4).trim() : target;
    }

    private String resolveMcpRouteCommand(String value) {
        if (value == null || value.trim().isEmpty()) return "";
        String exact = value.trim();
        if (tabKeyForCommand(exact) != null || "action_editor".equals(exact)
                || "expression_editor".equals(exact) || "action_variables".equals(exact)
                || isPacketChildCommand(exact)) return exact;
        String wanted = mcpSegment(exact);
        for (String candidate : tabRegistry.commands()) {
            if (wanted.equalsIgnoreCase(mcpSegment(candidate))) return candidate;
        }
        return exact;
    }

    private String normalizeMcpTabTarget(String value) {
        String target = value == null ? "" : value.trim().replace('\\', '/');
        String lower = target.toLowerCase(Locale.ROOT);
        int selectMarker = lower.indexOf("/tabs/select/");
        if (selectMarker >= 0) {
            String selected = target.substring(selectMarker + "/tabs/select/".length());
            int slash = selected.indexOf('/');
            return (slash < 0 ? selected : selected.substring(0, slash)).trim();
        }
        int tabMarker = lower.indexOf("/tab/");
        if (tabMarker >= 0) {
            String selected = target.substring(tabMarker + 5);
            int slash = selected.indexOf('/');
            return (slash < 0 ? selected : selected.substring(0, slash)).trim();
        }
        return target;
    ...
    private TabKey findTabKeyByMcpSegment(String value) {
        String wanted = mcpSegment(value);
        for (TabKey candidate : openTabs) {
            if (candidate == null) continue;
            if (wanted.equalsIgnoreCase(mcpSegment(candidate.encode()))
                    || wanted.equalsIgnoreCase(mcpSegment(commandForTabKey(candidate)))) {
                return candidate;
            }
        }
        return null;
    ...
    public void initGui() {
        ...
        if (tabStripInnerWidth <= 0) {
            tabStripInnerWidth = MainUiLayoutManager.getModernTabStripWidth();
        }
        if (tabStripHeight <= 0) {
            tabStripHeight = MainUiLayoutManager.getModernTabStripHeight();
        }
    ...
    private void drawHeader(ModernMainLayout.Layout layout, int mouseX, int mouseY) {
        ...
        if (layout.headerCollapsed) {
            drawCollapsedHeader(layout, mouseX, mouseY);
            searchBounds = null;
            return;
        }
        ModernMainLayout.Rect header = layout.header;
        ...
        List<ToolEntry> pinnedToolEntries = getPinnedToolEntries();
        int pinnedToolsWidth = headerToolWidth(pinnedToolEntries.size(), buttonSize, buttonGap);
        int pinnedToolsX = globalSettingsX - buttonGap - pinnedToolsWidth;
        int searchRight = (pinnedToolEntries.isEmpty() ? globalSettingsX : pinnedToolsX) - 7;
        ...
        for (int i = 0; i < pinnedToolEntries.size(); i++) {
            drawPinnedToolIcon(pinnedToolsX + i * (buttonSize + buttonGap), actionY, buttonSize, mouseX, mouseY,
                    pinnedToolEntries.get(i));
        }
    ...
    private void drawCollapsedHeader(ModernMainLayout.Layout layout, int mouseX, int mouseY) {
        ModernMainLayout.Rect header = layout.header;
        List<ToolEntry> pinnedToolEntries = getPinnedToolEntries();
        int actionSize = Math.max(1, Math.min(14, header.height));
        int actionGap = actionSize >= 12 ? 2 : 1;
        int actionCount = pinnedToolEntries.size() + COLLAPSED_HEADER_ACTION_TYPES.length;
        int actionWidth = actionCount * actionSize + (actionCount - 1) * actionGap;
        int availableActionWidth = Math.max(1, header.width - 12);
        if (actionCount > 0 && actionWidth > availableActionWidth) {
            actionGap = 1;
            actionSize = Math.max(1, (availableActionWidth - (actionCount - 1) * actionGap) / actionCount);
            actionWidth = actionCount * actionSize + (actionCount - 1) * actionGap;
        }
        int actionRight = header.right() - 6;
        int actionX = Math.max(header.x, actionRight - actionWidth);
        int actionY = header.y + (header.height - actionSize) / 2;

        // Keep the same reading order as the expanded header, while making
        // every entry a real hit target instead of hiding functionality.
        for (int i = 0; i < pinnedToolEntries.size(); i++) {
            drawPinnedToolIcon(actionX + i * (actionSize + actionGap), actionY, actionSize, mouseX, mouseY,
                    pinnedToolEntries.get(i));
        }
        for (int i = 0; i < COLLAPSED_HEADER_ACTION_TYPES.length; i++) {
            int actionIndex = i + pinnedToolEntries.size();
            TargetType type = COLLAPSED_HEADER_ACTION_TYPES[i];
            boolean active = type == TargetType.HEADER_SEARCH
                    && searchField != null && (searchField.isFocused() || !searchField.getText().isEmpty());
            if (type == TargetType.COMMAND_PALETTE) {
                active = commandPaletteOpen;
            } else if (type == TargetType.TOOLS) {
                active = toolsOpen;
            }
            drawHeaderIcon(actionX + actionIndex * (actionSize + actionGap), actionY, actionSize, mouseX, mouseY,
                    type, COLLAPSED_HEADER_ACTION_ICONS[i], active);
        }
        ...
        boolean versionMismatch = latestVersion.matches("[0-9]+(?:\\.[0-9]+)*")
                && !currentVersion.equals(latestVersion);
        String versionText = (versionMismatch ? TextFormatting.BOLD.toString() : "") + "v" + currentVersion
                + (versionMismatch ? " \u2192 " + latestVersion : "");
        int versionX = header.x + 8;
        int versionRight = Math.min(actionX - 7, layout.headerToggle.x - 7);
        int versionWidth = Math.max(1, versionRight - versionX);
        int versionY = header.y + Math.max(0, (header.height - fontRenderer.FONT_HEIGHT) / 2);
        drawHeaderVersion(versionText, versionMismatch, latestVersion, versionX, versionY, versionWidth, mouseX,
                mouseY);
        ModernTooltipSupport.registerTooltipAnchor(this, versionX, header.y, versionWidth, header.height,
                I18n.format("gui.changelog.header_versions", currentVersion, latestVersion) + "\n"
                        + tr("gui.modern.main.u130", "检查模组更新"));
    }

    private void drawHeaderToggle(ModernMainLayout.Layout layout, int mouseX, int mouseY) {
        if (layout == null || layout.headerToggle == null) {
            headerToggleBounds = null;
            return;
        }
        headerToggleBounds = layout.headerToggle;
        boolean hovered = headerToggleBounds.contains(mouseX, mouseY);
        int fill = hovered ? ModernUiRenderer.SURFACE_HOVER : ModernUiRenderer.SHELL_RAISED;
        ModernUiRenderer.drawSubtlePanel(headerToggleBounds.x, headerToggleBounds.y, headerToggleBounds.width,
                headerToggleBounds.height, 5, fill, hovered ? ModernUiRenderer.ACCENT_DIM : ModernUiRenderer.BORDER_SUBTLE);
        ModernUiRenderer.drawCollapseChevron(headerToggleBounds.x + headerToggleBounds.width / 2,
                headerToggleBounds.y + headerToggleBounds.height / 2, !layout.headerCollapsed,
                hovered ? ModernUiRenderer.TEXT : ModernUiRenderer.SUBTLE_TEXT);
        hitTargets.add(new HitTarget(TargetType.HEADER_TOGGLE, headerToggleBounds, ""));
        ModernTooltipSupport.registerTooltipAnchor(this, headerToggleBounds.x, headerToggleBounds.y,
                headerToggleBounds.width, headerToggleBounds.height,
                layout.headerCollapsed
                        ? tr("gui.modern.main.u147", "展开顶栏")
                        : tr("gui.modern.main.u146", "收起顶栏"));
    }

    private void drawHeaderIcon(int x, int y, int size, int mouseX, int mouseY, TargetType type, int icon) {
        drawHeaderIcon(x, y, size, mouseX, mouseY, type, icon, false);
    }
    ...
    private void drawHeaderIcon(int x, int y, int size, int mouseX, int mouseY, TargetType type, int icon,
        ...
        ModernUiRenderer.drawSubtlePanel(x, y, size, size, 5,
                ...
                active ? ModernUiRenderer.ACCENT
        ...
        int iconColor = type == TargetType.STOP_FOREGROUND ? ModernUiRenderer.ACCENT
                : type == TargetType.STOP_BACKGROUND ? ModernUiRenderer.WARNING
                        : hovered || active ? ModernUiRenderer.TEXT : ModernUiRenderer.SUBTLE_TEXT;
        ...
        if (icon == 0) {
            ...
            ModernUiRenderer.drawCommandIcon(iconX + 1, iconY + 1, iconColor);
        } else if (icon == 8) {
            ModernUiRenderer.drawSearchIcon(iconX + 1, iconY + 1, iconColor);
        } else {
            ModernUiRenderer.drawCloseIcon(iconX + 2, iconY + 2, iconColor);
        }
        hitTargets.add(new HitTarget(type, bounds, ""));
        ModernTooltipSupport.registerTooltipAnchor(this, bounds.x, bounds.y, bounds.width, bounds.height,
                headerTooltip(type));
    }

    private String headerTooltip(TargetType type) {
        switch (type) {
            case STOP_FOREGROUND:
                return "gui.modern.main.u004";
            case STOP_BACKGROUND:
                return "gui.modern.main.u005";
            case PATHS:
                return "gui.modern.main.u006";
            case TOOLS:
                return "gui.modern.main.u007";
            case DETACH:
                return DetachedSwingWindowManager.isDetachedScreen(this)
                        ? "gui.modern.main.u008" : "gui.modern.main.u009";
            case CLOSE:
                return "gui.modern.main.u010";
            case GLOBAL_SETTINGS:
                return "gui.modern.main.u011";
            case HEADER_SEARCH:
                return "gui.modern.main.u003";
            case COMMAND_PALETTE:
                return tr("gui.modern.main.palette.title", "打开命令面板") + "\n"
                        + tr("gui.modern.main.palette.desc", "搜索并执行功能、路径和快捷动作。") + "\n"
                        + formatKeybindLabel(KeybindManager.keybinds.get(BindableAction.OPEN_COMMAND_PALETTE));
            default:
                return "";
        }
    }

    private void drawPinnedToolIcon(int x, int y, int size, int mouseX, int mouseY, ToolEntry entry) {
        ...
        ModernUiRenderer.drawSubtlePanel(x, y, size, size, 5,
                hovered ? ModernUiRenderer.SURFACE_HOVER : ModernUiRenderer.SHELL_RAISED,
                hovered ? 0xFF637783 : ModernUiRenderer.BORDER_SUBTLE);
        int iconSize = Math.max(1, Math.min(14, size - 4));
        int iconColor = hovered ? ModernUiRenderer.TEXT : ModernUiRenderer.SUBTLE_TEXT;
        ModernUiRenderer.drawIcon(getToolIcon(entry.action), x + (size - iconSize) / 2,
                y + (size - iconSize) / 2, iconSize, iconColor);
        hitTargets.add(new HitTarget(TargetType.UTILITY, bounds, entry.action));
        ModernTooltipSupport.registerTooltipAnchor(this, bounds.x, bounds.y, bounds.width, bounds.height,
                entry.title + "\n" + getToolDescription(entry.action));
    }

    private int headerToolWidth(int count, int buttonSize, int gap) {
        if (count <= 0) {
            return 0;
        }
    ...
    private ModernUiRenderer.Icon getToolIcon(String action) {
        if ("theme".equals(action)) {
            return ModernUiRenderer.Icon.SETTINGS;
        }
        if ("reset_window".equals(action)) {
            return ModernUiRenderer.Icon.DISPLAY;
        }
        if ("update".equals(action)) {
            return ModernUiRenderer.Icon.RELOAD;
        }
        if ("changelog".equals(action)) {
            return ModernUiRenderer.Icon.INFO;
        }
        if ("donate".equals(action)) {
            return ModernUiRenderer.Icon.PROFILE;
        }
        if ("mcp_settings".equals(action)) {
        ...
        for (int i = 0; i < rows.size(); i++) {
            ...
            if (collapsed) {
                ...
                if (navigationRow.otherFeatureGroup) {
                    ...
                    boolean hudIconHovered = hudIcon.contains(mouseX, mouseY);
                    int iconSize = hudIconHovered ? 16 : 12;
                    int iconX = hudIcon.x + (hudIcon.width - iconSize) / 2;
                    int iconY = hudIcon.y + (hudIcon.height - iconSize) / 2;
                    int iconColor = ModernUiRenderer.readableText(ModernUiRenderer.TEXT, fill);
                    if (hudIconHovered) {
                        float rotation = (System.currentTimeMillis() % 1200L) * 0.3F;
                        ModernUiRenderer.drawIconRotated(ModernUiRenderer.Icon.MOVE_PANEL, iconX, iconY,
                                iconSize, iconColor, rotation);
                    } else {
                        ModernUiRenderer.drawIcon(ModernUiRenderer.Icon.MOVE_PANEL, iconX, iconY, iconSize, iconColor);
                    }
    ...
    private void drawTabStripDivider(ModernTabStripLayout.Layout stripLayout, int mouseX, int mouseY) {
        if (stripLayout == null) {
            tabStripDividerBounds = null;
            return;
        }
        if (stripLayout.vertical()) {
            tabStripDividerBounds = ModernSplitPane.verticalDividerBounds(stripLayout.strip.x, stripLayout.strip.width, 0,
                    stripLayout.strip.y, Math.max(1, stripLayout.strip.height));
            ModernSplitPane.drawVerticalDivider(tabStripDividerBounds, mouseX, mouseY, draggingTabStrip);
            ModernTooltipSupport.registerTooltipAnchor(this, tabStripDividerBounds.x, tabStripDividerBounds.y,
                    tabStripDividerBounds.width, tabStripDividerBounds.height,
                    tr("gui.modern.resize_tabs", "gui.modern.main.u135"));
        } else {
            tabStripDividerBounds = ModernSplitPane.horizontalDividerBounds(stripLayout.strip.x,
                    stripLayout.strip.width, stripLayout.strip.y, stripLayout.strip.height,
                    Math.max(0, stripLayout.body.y - stripLayout.strip.bottom()));
            ModernSplitPane.drawHorizontalDivider(tabStripDividerBounds, mouseX, mouseY, draggingTabStripHeight);
            ModernTooltipSupport.registerTooltipAnchor(this, tabStripDividerBounds.x, tabStripDividerBounds.y,
                    tabStripDividerBounds.width, tabStripDividerBounds.height,
                    tr("gui.modern.resize_tab_height", "gui.modern.main.u145"));
        }
    }

    private void drawContent(ModernMainLayout.Layout layout, int mouseX, int mouseY) {
        ModernSettingsTab settingsTab = getActiveSettingsTab();
        if (isActiveTabFullscreen() && settingsTab != null) {
            lastTabStrip = null;
            tabStripBounds = null;
            tabStripDividerBounds = null;
            headerToggleBounds = null;
            ...
            ModernMainLayout.Rect fullscreenBounds = new ModernMainLayout.Rect(layout.shell.x,
                    layout.header.bottom(), layout.shell.width,
                    Math.max(1, layout.shell.bottom() - layout.header.bottom()));
            drawSettingsTab(settingsTab, fullscreenBounds, mouseX, mouseY);
            drawDashboardSettingsPopover(layout, null, mouseX, mouseY);
            return;
        }
        lastTabStrip = buildTabStripLayout(layout.content);
        tabScrollbar = lastTabStrip.vertical() ? verticalTabScrollbar : horizontalTabScrollbar;
        drawTabs(lastTabStrip, mouseX, mouseY);
        drawTabReorderGhost(mouseX, mouseY);
        drawTabStripDivider(lastTabStrip, mouseX, mouseY);
        if (settingsTab != null) {
            drawSettingsTab(settingsTab, lastTabStrip.body, mouseX, mouseY);
        } else {
            ModernMainLayout.Rect dashboardBody = prepareDashboardSettingsLayout(layout, lastTabStrip);
            drawDashboardContent(layout, dashboardBody, mouseX, mouseY);
        }
        drawDashboardSettingsPopover(layout, lastTabStrip, mouseX, mouseY);
    }

    private void drawSettingsTab(ModernSettingsTab settingsTab, ModernMainLayout.Rect bounds, int mouseX, int mouseY) {
        contentClipBounds = null;
        settingsTab.draw(fontRenderer, bounds, mouseX, mouseY);
        boolean restoredAfterLayout = restorePageStateAfterFirstDraw.remove(settingsTab);
        if (restoredAfterLayout) {
            restorePageState(stateCommandForTab(activeTab), settingsTab);
        }
        String focusTarget = pendingTabFocusAfterStateRestore.remove(settingsTab);
        if (focusTarget != null && !focusTarget.isEmpty()) {
            // State restoration happens after the first layout. Re-submit a
            // requested row target so it wins on the following frame.
            settingsTab.focusCommand(focusTarget);
        }
        if (!restoredAfterLayout) {
            capturePageState(stateCommandForTab(activeTab), settingsTab);
        }
    ...
    private ModernMainLayout.Rect prepareDashboardSettingsLayout(ModernMainLayout.Layout layout,
            ModernTabStripLayout.Layout stripLayout) {
        ModernMainLayout.Rect body = stripLayout == null ? layout.content : stripLayout.body;
        dashboardSettingsResizeBounds = null;
        if (!dashboardSettingsOpen || !isDashboardSettingsEmbedded() || body == null) {
            return body;
        }
        int panelWidth = getDashboardSettingsPanelWidth(body);
        int gap = 6;
        int contentWidth = Math.max(1, body.width - panelWidth - gap);
        int panelX = MainUiLayoutManager.DASHBOARD_SETTINGS_LEFT.equals(getDashboardSettingsPlacement())
                ? body.x : body.right() - panelWidth;
        int contentX = MainUiLayoutManager.DASHBOARD_SETTINGS_LEFT.equals(getDashboardSettingsPlacement())
                ? panelX + panelWidth + gap : body.x;
        dashboardSettingsPanelBounds = new ModernMainLayout.Rect(panelX, body.y, panelWidth, body.height);
        dashboardSettingsResizeBounds = new ModernMainLayout.Rect(
                MainUiLayoutManager.DASHBOARD_SETTINGS_LEFT.equals(getDashboardSettingsPlacement())
                        ? panelX + panelWidth - 3 : panelX - 3,
                body.y, 6, body.height);
        return new ModernMainLayout.Rect(contentX, body.y, contentWidth, body.height);
    }

    private boolean isDashboardSettingsEmbedded() {
        String placement = getDashboardSettingsPlacement();
        return MainUiLayoutManager.DASHBOARD_SETTINGS_LEFT.equals(placement)
                || MainUiLayoutManager.DASHBOARD_SETTINGS_RIGHT.equals(placement);
    }

    private String getDashboardSettingsPlacement() {
        return MainUiLayoutManager.getModernDashboardSettingsPlacement();
    }

    private int getEffectiveDashboardSettingsWidth() {
        return draggingDashboardSettingsResize ? dashboardSettingsResizePreviewWidth
                : MainUiLayoutManager.getModernDashboardSettingsWidth();
    }

    private int getDashboardSettingsPanelWidth(ModernMainLayout.Rect body) {
        int available = body == null ? this.width - 12 : body.width;
        int maxWidth = Math.max(1, available - 84);
        int minWidth = Math.min(MainUiLayoutManager.MIN_DASHBOARD_SETTINGS_WIDTH, maxWidth);
        return clamp(getEffectiveDashboardSettingsWidth(), minWidth, maxWidth);
    }

    private void drawTabs(ModernTabStripLayout.Layout stripLayout, int mouseX, int mouseY) {
        ModernMainLayout.Rect strip = stripLayout.strip;
        ModernMainLayout.Rect tabViewport = stripLayout.viewport;
        tabStripBounds = stripLayout.vertical() ? strip
                : new ModernMainLayout.Rect(strip.x, strip.y, strip.width,
                        strip.height + ModernHoverScrollbar.HORIZONTAL_GUTTER);
        hitTargets.add(new HitTarget(TargetType.TAB_STRIP, tabStripBounds, ""));
        if (!stripLayout.vertical()) {
            ModernUiRenderer.drawDivider(stripLayout.body.x + 12, stripLayout.body.y - 1,
                    Math.max(1, stripLayout.body.width - 24), ModernUiRenderer.BORDER_SUBTLE);
        }
        dashboardSettingsGearBounds = stripLayout.gear;
        ...
        int gearIconX = dashboardSettingsGearBounds.x + (dashboardSettingsGearBounds.width - 12) / 2;
        int gearIconY = dashboardSettingsGearBounds.y + (dashboardSettingsGearBounds.height - 12) / 2;
        ModernUiRenderer.drawSettingsIconRotated(gearIconX, gearIconY, ModernUiRenderer.readableText(
                gearHovered || dashboardSettingsOpen ? ModernUiRenderer.TEXT : ModernUiRenderer.SUBTLE_TEXT, gearFill),
        ...
        for (int i = 0; i < openTabs.size(); i++) {
            ...
            if (active) {
                if (stripLayout.vertical()) {
                    ModernUiRenderer.drawRoundedRect(tabBounds.x + 2, tabBounds.y + 5, 2, tabBounds.height - 10, 1,
            ...
            int titleWidth = tabBounds.width - (tab.isDashboard() ? ModernTabStripLayout.DASHBOARD_CHROME
                    : ModernTabStripLayout.CLOSE_CHROME);
            ModernUiRenderer.drawText(fontRenderer, title, tabBounds.x + ModernTabStripLayout.TITLE_INSET,
            ...
            if (!tab.isDashboard()) {
                ModernMainLayout.Rect closeBounds = new ModernMainLayout.Rect(
                        tabBounds.right() - ModernTabStripLayout.CLOSE_BUTTON_SLOT,
        ...
        // Keep tab decorations in the same scissor region as the tab bodies.
        // Dependency frames are based on the unscrolled tab coordinates, so a
        // child that is partly outside the viewport can otherwise paint its
        // connector over the surrounding shell/sidebar.
        drawTabReorderDropLine(stripLayout);
        drawDependencyFrames(tabViewport, renderedTabBounds, mouseX, mouseY, stripLayout.vertical());
        ModernUiRenderer.endClip();
        if (maxTabScrollOffset > 0) {
            ModernMainLayout.Rect scrollViewport = stripLayout.scrollViewport();
            tabScrollbar.draw(scrollViewport, tabScrollOffset, maxTabScrollOffset, stripLayout.visibleExtent(),
                    Math.max(stripLayout.visibleExtent(), stripLayout.totalExtent), mouseX, mouseY,
                    value -> tabScrollOffset = value);
            tabScrollbarBounds = scrollViewport;
    ...
    private void drawDependencyFrames(ModernMainLayout.Rect viewport,
            Map<TabKey, ModernMainLayout.Rect> renderedTabBounds, int mouseX, int mouseY, boolean vertical) {
        ...
        for (TabKey tab : openTabs) {
            ...
            List<ModernMainLayout.Rect> visibleMembers = new ArrayList<>();
            for (ModernMainLayout.Rect member : members) {
                if (intersects(member, viewport)) {
            ...
            drawnRoots.add(root);
            // Keep the full group's extents so a partially visible group still
            // reaches the viewport edge; the active clip trims the off-screen
            // portion without shortening the visible frame prematurely.
            int left = members.get(0).x;
            int right = members.get(0).right();
            int top = members.get(0).y;
            int bottom = members.get(0).bottom();
            for (ModernMainLayout.Rect member : members) {
            ...
            int frameTop = Math.max(viewport.y, top - 2);
            int frameBottom = Math.min(viewport.bottom(), bottom + 1);
            if (end <= x || frameBottom <= frameTop) {
                continue;
            }
            int color = 0xFF6D9FB0;
            Gui.drawRect(x, frameTop, end, frameTop + 1, color);
            Gui.drawRect(x, frameBottom - 1, end, frameBottom, color);
            Gui.drawRect(x, frameTop, x + 1, frameBottom, color);
            Gui.drawRect(end - 1, frameTop, end, frameBottom, color);
            for (int i = 0; i + 1 < openTabs.size(); i++) {
                ...
                if (root.equals(dependencyRoot(current)) && root.equals(dependencyRoot(next))) {
                    ...
                    if (currentBounds != null && nextBounds != null) {
                        if (vertical) {
                            int connectorTop = Math.max(viewport.y, currentBounds.bottom());
                            int connectorBottom = Math.min(viewport.bottom(), nextBounds.y);
                            int connectorLeft = Math.max(viewport.x, x + 3);
                            int connectorRight = Math.min(viewport.right(), end - 3);
                            if (connectorBottom > connectorTop && connectorRight > connectorLeft) {
                                Gui.drawRect(connectorLeft, connectorTop, connectorRight, connectorBottom, color);
                            }
                        } else {
                            int connectorLeft = Math.max(viewport.x, currentBounds.right());
                            int connectorRight = Math.min(viewport.right(), nextBounds.x);
                            int connectorTop = Math.max(viewport.y, frameTop + 3);
                            int connectorBottom = Math.min(viewport.bottom(), frameBottom - 3);
                            if (connectorRight > connectorLeft && connectorBottom > connectorTop) {
                                Gui.drawRect(connectorLeft, connectorTop, connectorRight, connectorBottom, color);
                            }
                        }
                    }
            ...
            if (mouseX >= x && mouseX <= end && mouseY >= frameTop && mouseY <= frameBottom) {
    ...
    private void drawDashboardContent(ModernMainLayout.Layout layout, ModernMainLayout.Rect body, int mouseX, int mouseY) {
        ModernMainLayout.Rect content = body == null ? layout.content : body;
        ...
        int cardWidth = getDashboardCardWidth(view);
        int requestedColumns = view.columns;
        ModernMainLayout.Grid grid = ModernMainLayout.createDashboardGrid(content, 4,
                paged || showAutoPause ? 27 : 8, 7, requestedColumns, cardHeight, cardWidth);
    ...
    private void drawDashboardSettingsPopover(ModernMainLayout.Layout layout, ModernTabStripLayout.Layout stripLayout,
            int mouseX, int mouseY) {
        if (!dashboardSettingsOpen || dashboardSettingsGearBounds == null) {
            dashboardSettingsPanelBounds = null;
            dashboardSettingsViewportBounds = null;
            dashboardSettingsResizeBounds = null;
        ...
        ModernMainLayout.Rect body = stripLayout == null ? layout.content : stripLayout.body;
        boolean embedded = isDashboardSettingsEmbedded();
        String[] keys = { "placement", "panel_width", "sort", "grouping", "icons", "size", "width", "height",
                "text_size", "text_align", "text_vertical", "info", "columns", "flow" };
        int maxScreenWidth = Math.max(1, this.width - 12);
        int panelWidth = Math.min(getDashboardSettingsPanelWidth(body), maxScreenWidth);
        int fullPanelHeight = 44 + keys.length * 27 + 15;
        int panelHeight = Math.min(fullPanelHeight, Math.max(1, this.height - 12));
        int panelX;
        int panelY;
        if (embedded && dashboardSettingsPanelBounds != null) {
            panelX = dashboardSettingsPanelBounds.x;
            panelY = dashboardSettingsPanelBounds.y;
            panelWidth = dashboardSettingsPanelBounds.width;
            panelHeight = dashboardSettingsPanelBounds.height;
        } else if (stripLayout != null && stripLayout.vertical()) {
            panelX = clamp(dashboardSettingsGearBounds.right() + 6, body.x,
                    Math.max(body.x, body.right() - panelWidth));
            panelY = clamp(dashboardSettingsGearBounds.y, 6, Math.max(6, this.height - panelHeight - 6));
        } else {
            panelX = clamp(dashboardSettingsGearBounds.x, body.x + 6,
                    Math.max(body.x + 6, body.right() - panelWidth - 6));
            panelY = clamp(body.y + 3, 6, Math.max(6, this.height - panelHeight - 6));
        }
        panelX = clamp(panelX, 6, Math.max(6, this.width - panelWidth - 6));
        dashboardSettingsPanelBounds = new ModernMainLayout.Rect(panelX, panelY, panelWidth, panelHeight);
        if (!embedded) {
            dashboardSettingsResizeBounds = new ModernMainLayout.Rect(panelX + panelWidth - 3, panelY, 6, panelHeight);
        }
        ...
        if (dashboardSettingsResizeBounds != null) {
            boolean resizeHovered = dashboardSettingsResizeBounds.contains(mouseX, mouseY);
            ModernSplitPane.drawVerticalDivider(dashboardSettingsResizeBounds, mouseX, mouseY,
                    draggingDashboardSettingsResize || resizeHovered);
        }
        ...
        String[] titles = { tr("gui.modern.dashboard.settings_placement", "设置位置"),
                tr("gui.modern.dashboard.settings_width", "面板宽度"),
                tr("gui.modern.sort", "gui.modern.main.u034"), tr("gui.modern.grouping", "gui.modern.main.u035"),
                tr("gui.modern.dashboard.show_icons", "显示图标"),
                tr("gui.modern.icon_size", "gui.modern.main.u036"),
                tr("gui.modern.dashboard.card_width", "按键宽度"),
                tr("gui.modern.dashboard.card_height", "按键高度"),
                tr("gui.modern.dashboard.text_size", "文字大小"),
                tr("gui.modern.dashboard.text_align", "水平对齐"),
                tr("gui.modern.dashboard.text_vertical", "垂直对齐"),
                tr("gui.modern.dashboard.info_mode", "提示显示"),
                tr("gui.modern.columns", "gui.modern.main.u037"),
                tr("gui.modern.flow", "gui.modern.main.u038") };
        String[] values = {
                getDashboardSettingsPlacementLabel(),
                getDashboardSettingsWidthLabel(),
                getDashboardSortLabel(view),
                view.grouping ? tr("gui.modern.on", "gui.modern.main.u039") : tr("gui.modern.off", "gui.modern.main.u040"),
                view.showIcons ? tr("gui.modern.on", "gui.modern.main.u039") : tr("gui.modern.off", "gui.modern.main.u040"),
                getDashboardIconSizeLabel(view),
                getDashboardCardWidthLabel(view),
                getDashboardCardHeightLabel(view),
                getDashboardTextScaleLabel(view),
                getDashboardTextAlignLabel(view),
                getDashboardTextVerticalAlignLabel(view),
                getDashboardInfoModeLabel(view),
        ...
        String[] tooltips = {
                "gui.modern.dashboard.settings_placement_tip",
                "gui.modern.dashboard.settings_width_tip",
                "gui.modern.main.u043",
                "gui.modern.main.u044",
                "gui.modern.dashboard.show_icons_tip",
                "gui.modern.main.u045",
                "gui.modern.dashboard.card_width_tip",
                "gui.modern.dashboard.card_height_tip",
                "gui.modern.dashboard.text_size_tip",
                "gui.modern.dashboard.text_align_tip",
                "gui.modern.dashboard.text_vertical_tip",
                "gui.modern.dashboard.info_mode_tip",
                "gui.modern.main.u046",
                "gui.modern.main.u047"
        };
        int settingsContentWidth = ModernHoverScrollbar.contentWidth(innerWidth);
        ModernMainLayout.Rect settingsViewport = new ModernMainLayout.Rect(innerX, panelY + 44,
                settingsContentWidth, Math.max(1, panelHeight - 52));
        dashboardSettingsViewportBounds = settingsViewport;
        int settingsContentHeight = keys.length * 27;
        dashboardSettingsMaxScroll = Math.max(0, settingsContentHeight - settingsViewport.height);
        dashboardSettingsScroll = clamp(dashboardSettingsScroll, 0, dashboardSettingsMaxScroll);
        ModernMainLayout.Rect[] rows = new ModernMainLayout.Rect[keys.length];
        for (int i = 0; i < keys.length; i++) {
            rows[i] = new ModernMainLayout.Rect(settingsViewport.x,
                    settingsViewport.y + i * 27 - dashboardSettingsScroll, settingsViewport.width, 23);
        }
        ...
        ModernUiRenderer.beginClip(settingsViewport);
        for (int i = 0; i < keys.length; i++) {
            ModernMainLayout.Rect row = rows[i];
            boolean visible = intersects(row, settingsViewport);
            boolean expanded = keys[i].equals(hoveredTool);
            boolean hovered = visible && (expanded || row.contains(mouseX, mouseY));
            if (expanded && visible) {
                hoveredRow = row;
            }
            ...
            if (visible) {
                ModernMainLayout.Rect hit = intersection(row, settingsViewport);
                hitTargets.add(new HitTarget(TargetType.DASHBOARD_TOOL, hit, keys[i]));
                if (!expanded) {
                    ModernTooltipSupport.registerTooltipAnchor(this, hit.x, hit.y, hit.width, hit.height, tooltips[i]);
                }
            }
        }
        ModernUiRenderer.endClip();
        dashboardSettingsScrollbar.drawAt(innerX + innerWidth, settingsViewport.y, settingsViewport.height,
                dashboardSettingsScroll, dashboardSettingsMaxScroll, settingsViewport.height, settingsContentHeight,
                mouseX, mouseY,
                value -> dashboardSettingsScroll = value);
    ...
    private String resolveDashboardSettingsHoverTool(String[] keys, ModernMainLayout.Rect[] rows, int mouseX,
            int mouseY) {
        if (dashboardSettingsViewportBounds != null
                && !dashboardSettingsViewportBounds.contains(mouseX, mouseY)) {
            // The submenu and its bridge are checked below so they remain reachable
            // while the pointer is moving outside the clipped row viewport.
        } else {
    ...
    private void drawDashboardSettingsSubmenu(String tool, ModernMainLayout.Rect row,
        ...
        for (int i = 0; i < options.size(); i++) {
            String label = tr(options.get(i).label, options.get(i).label);
            maxLabelWidth = Math.max(maxLabelWidth, fontRenderer.getStringWidth(label));
        }
        int menuWidth = Math.max(112, Math.min(220, maxLabelWidth + 38));
    ...
    private List<DashboardToolOption> getDashboardToolOptions(String tool) {
        ...
        if ("placement".equals(tool)) {
            options.add(new DashboardToolOption(MainUiLayoutManager.DASHBOARD_SETTINGS_FLOATING,
                    "gui.modern.dashboard.settings_floating"));
            options.add(new DashboardToolOption(MainUiLayoutManager.DASHBOARD_SETTINGS_LEFT,
                    "gui.modern.dashboard.settings_left"));
            options.add(new DashboardToolOption(MainUiLayoutManager.DASHBOARD_SETTINGS_RIGHT,
                    "gui.modern.dashboard.settings_right"));
        } else if ("panel_width".equals(tool)) {
            for (int width : DASHBOARD_SETTINGS_WIDTH_OPTIONS) {
                options.add(new DashboardToolOption(String.valueOf(width), width + " px"));
            }
            options.add(new DashboardToolOption("custom", "gui.modern.dashboard.settings_width_custom"));
        ...
        } else if ("width".equals(tool)) {
            options.add(new DashboardToolOption("0", "gui.modern.dashboard.card_width_auto"));
            options.add(new DashboardToolOption("140", "gui.modern.dashboard.card_width_140"));
            options.add(new DashboardToolOption("180", "gui.modern.dashboard.card_width_180"));
            options.add(new DashboardToolOption("220", "gui.modern.dashboard.card_width_220"));
            options.add(new DashboardToolOption("260", "gui.modern.dashboard.card_width_260"));
            options.add(new DashboardToolOption("300", "gui.modern.dashboard.card_width_300"));
            options.add(new DashboardToolOption("360", "gui.modern.dashboard.card_width_360"));
            options.add(new DashboardToolOption("custom", "gui.modern.dashboard.card_width_custom"));
        } else if ("height".equals(tool)) {
            options.add(new DashboardToolOption("0", "gui.modern.dashboard.card_height_auto"));
            options.add(new DashboardToolOption("40", "gui.modern.dashboard.card_height_40"));
            options.add(new DashboardToolOption("56", "gui.modern.dashboard.card_height_56"));
            options.add(new DashboardToolOption("72", "gui.modern.dashboard.card_height_72"));
            options.add(new DashboardToolOption("96", "gui.modern.dashboard.card_height_96"));
            options.add(new DashboardToolOption("120", "gui.modern.dashboard.card_height_120"));
            options.add(new DashboardToolOption("144", "gui.modern.dashboard.card_height_144"));
            options.add(new DashboardToolOption("custom", "gui.modern.dashboard.card_height_custom"));
        } else if ("text_size".equals(tool)) {
            for (int percent : DASHBOARD_TEXT_SCALE_OPTIONS) {
                options.add(new DashboardToolOption(String.valueOf(percent), percent + "%"));
            }
            options.add(new DashboardToolOption("custom", "gui.modern.dashboard.text_size_custom"));
        } else if ("text_align".equals(tool)) {
            options.add(new DashboardToolOption(MainUiLayoutManager.DASHBOARD_TEXT_ALIGN_LEFT,
                    "gui.modern.dashboard.align_left"));
            options.add(new DashboardToolOption(MainUiLayoutManager.DASHBOARD_TEXT_ALIGN_CENTER,
                    "gui.modern.dashboard.align_center"));
            options.add(new DashboardToolOption(MainUiLayoutManager.DASHBOARD_TEXT_ALIGN_RIGHT,
                    "gui.modern.dashboard.align_right"));
        } else if ("text_vertical".equals(tool)) {
            options.add(new DashboardToolOption(MainUiLayoutManager.DASHBOARD_TEXT_VERTICAL_TOP,
                    "gui.modern.dashboard.align_top"));
            options.add(new DashboardToolOption(MainUiLayoutManager.DASHBOARD_TEXT_VERTICAL_CENTER,
                    "gui.modern.dashboard.align_center_vertical"));
            options.add(new DashboardToolOption(MainUiLayoutManager.DASHBOARD_TEXT_VERTICAL_BOTTOM,
                    "gui.modern.dashboard.align_bottom"));
        } else if ("info".equals(tool)) {
            options.add(new DashboardToolOption(MainUiLayoutManager.DASHBOARD_INFO_ICON,
                    "gui.modern.dashboard.info_icon"));
            options.add(new DashboardToolOption(MainUiLayoutManager.DASHBOARD_INFO_INLINE,
                    "gui.modern.dashboard.info_inline"));
            options.add(new DashboardToolOption(MainUiLayoutManager.DASHBOARD_INFO_HIDDEN,
                    "gui.modern.dashboard.info_hidden"));
    ...
    private String getDashboardToolSelectedId(String tool, MainUiLayoutManager.DashboardViewMeta view) {
        ...
        if ("placement".equals(tool)) {
            return getDashboardSettingsPlacement();
        }
        if ("panel_width".equals(tool)) {
            return dashboardDimensionOptionId(getEffectiveDashboardSettingsWidth(), DASHBOARD_SETTINGS_WIDTH_OPTIONS);
        }
        if ("sort".equals(tool)) {
        ...
        if ("icons".equals(tool)) {
            return view.showIcons ? "on" : "off";
        }
        if ("size".equals(tool)) {
            return MainUiLayoutManager.ICON_XL.equals(view.iconSize)
                    ? MainUiLayoutManager.ICON_LARGE : view.iconSize;
        }
        if ("width".equals(tool)) {
            return dashboardDimensionOptionId(view.cardWidth, DASHBOARD_CARD_WIDTH_OPTIONS);
        }
        if ("height".equals(tool)) {
            return dashboardDimensionOptionId(view.cardHeight, DASHBOARD_CARD_HEIGHT_OPTIONS);
        }
        if ("text_size".equals(tool)) {
            return dashboardTextScaleOptionId(view.textScalePercent);
        }
        if ("text_align".equals(tool)) {
            return view.textAlign;
        }
        if ("text_vertical".equals(tool)) {
            return view.textVerticalAlign;
        }
        if ("info".equals(tool)) {
            return view.infoMode;
        }
    ...
    private String dashboardDimensionOptionId(int value, int[] presets) {
        for (int preset : presets) {
            if (preset == value) {
                return String.valueOf(preset);
            }
        }
        return value > 0 ? "custom" : "0";
    }

    private String dashboardTextScaleOptionId(int value) {
        for (int preset : DASHBOARD_TEXT_SCALE_OPTIONS) {
            if (preset == value) {
                return String.valueOf(preset);
            }
        }
    ...
    private void drawDashboardToolIcon(String key, int x, int y, int color) {
        ModernUiRenderer.Icon icon = "placement".equals(key) ? ModernUiRenderer.Icon.MOVE_PANEL
                : "sort".equals(key) ? ModernUiRenderer.Icon.SORT
                : "grouping".equals(key) ? ModernUiRenderer.Icon.GROUP
                        : "icons".equals(key) || "info".equals(key) ? ModernUiRenderer.Icon.INFO
                                : "panel_width".equals(key) || "size".equals(key) || "width".equals(key)
                                        || "height".equals(key)
                                        ? ModernUiRenderer.Icon.SIZE
    ...
    private int getDashboardCardHeight(MainUiLayoutManager.DashboardViewMeta view) {
        if (view != null && view.cardHeight > 0) {
            return Math.max(MainUiLayoutManager.MIN_DASHBOARD_CARD_HEIGHT,
                    Math.min(MainUiLayoutManager.MAX_DASHBOARD_CARD_HEIGHT, view.cardHeight));
        }
    ...
    private int getDashboardCardWidth(MainUiLayoutManager.DashboardViewMeta view) {
        if (view == null || view.cardWidth <= 0) {
            return MainUiLayoutManager.DASHBOARD_CARD_DIMENSION_AUTO;
        }
        return Math.max(MainUiLayoutManager.MIN_DASHBOARD_CARD_WIDTH,
                Math.min(MainUiLayoutManager.MAX_DASHBOARD_CARD_WIDTH, view.cardWidth));
    }
    ...
    private String getDashboardCardWidthLabel(MainUiLayoutManager.DashboardViewMeta view) {
        int width = view == null ? MainUiLayoutManager.DASHBOARD_CARD_DIMENSION_AUTO : view.cardWidth;
        return width <= 0 ? tr("gui.modern.dashboard.card_width_auto", "自动适配") : width + " px";
    }

    private String getDashboardCardHeightLabel(MainUiLayoutManager.DashboardViewMeta view) {
        int height = view == null ? MainUiLayoutManager.DASHBOARD_CARD_DIMENSION_AUTO : view.cardHeight;
        return height <= 0 ? tr("gui.modern.dashboard.card_height_auto", "自动适配") : height + " px";
    }

    private String getDashboardTextScaleLabel(MainUiLayoutManager.DashboardViewMeta view) {
        int percent = view == null ? MainUiLayoutManager.DEFAULT_DASHBOARD_TEXT_SCALE_PERCENT
                : view.textScalePercent;
        return percent + "%";
    }

    private String getDashboardTextAlignLabel(MainUiLayoutManager.DashboardViewMeta view) {
        String align = view == null ? MainUiLayoutManager.DASHBOARD_TEXT_ALIGN_CENTER : view.textAlign;
        if (MainUiLayoutManager.DASHBOARD_TEXT_ALIGN_LEFT.equals(align)) {
            return tr("gui.modern.dashboard.align_left", "左对齐");
        }
        if (MainUiLayoutManager.DASHBOARD_TEXT_ALIGN_RIGHT.equals(align)) {
            return tr("gui.modern.dashboard.align_right", "右对齐");
        }
        return tr("gui.modern.dashboard.align_center", "居中");
    }

    private String getDashboardTextVerticalAlignLabel(MainUiLayoutManager.DashboardViewMeta view) {
        String align = view == null ? MainUiLayoutManager.DASHBOARD_TEXT_VERTICAL_CENTER : view.textVerticalAlign;
        if (MainUiLayoutManager.DASHBOARD_TEXT_VERTICAL_TOP.equals(align)) {
            return tr("gui.modern.dashboard.align_top", "顶部");
        }
        if (MainUiLayoutManager.DASHBOARD_TEXT_VERTICAL_BOTTOM.equals(align)) {
            return tr("gui.modern.dashboard.align_bottom", "底部");
        }
        return tr("gui.modern.dashboard.align_center_vertical", "居中");
    }

    private String getDashboardSettingsPlacementLabel() {
        String placement = getDashboardSettingsPlacement();
        if (MainUiLayoutManager.DASHBOARD_SETTINGS_LEFT.equals(placement)) {
            return tr("gui.modern.dashboard.settings_left", "左侧嵌入");
        }
        if (MainUiLayoutManager.DASHBOARD_SETTINGS_RIGHT.equals(placement)) {
            return tr("gui.modern.dashboard.settings_right", "右侧嵌入");
        }
        return tr("gui.modern.dashboard.settings_floating", "悬浮");
    }

    private String getDashboardSettingsWidthLabel() {
        return getEffectiveDashboardSettingsWidth() + " px";
    }

    private String getDashboardInfoModeLabel(MainUiLayoutManager.DashboardViewMeta view) {
        String mode = view == null ? MainUiLayoutManager.DASHBOARD_INFO_ICON : view.infoMode;
        if (MainUiLayoutManager.DASHBOARD_INFO_INLINE.equals(mode)) {
            return tr("gui.modern.dashboard.info_inline", "卡片内文字");
        }
        if (MainUiLayoutManager.DASHBOARD_INFO_HIDDEN.equals(mode)) {
            return tr("gui.modern.dashboard.info_hidden", "不显示");
        }
        return tr("gui.modern.dashboard.info_icon", "信息图标");
    }
    ...
    private void drawFeatureCard(MenuCard card, ModernMainLayout.Rect bounds, ModernMainLayout.Rect clipBounds, int mouseX,
        ...
        MainUiLayoutManager.DashboardViewMeta view = getDashboardView();
        boolean showIcons = view == null || view.showIcons;
        boolean sequenceCard = isPathSequenceCommand(card.command);
        String infoMode = view == null ? MainUiLayoutManager.DASHBOARD_INFO_ICON : view.infoMode;
        boolean showInfo = MainUiLayoutManager.DASHBOARD_INFO_ICON.equals(infoMode)
                && bounds.width >= 80;
        boolean showInlineInfo = MainUiLayoutManager.DASHBOARD_INFO_INLINE.equals(infoMode);
        boolean prominent = bounds.height >= 84;
        float textScale = (view == null ? MainUiLayoutManager.DEFAULT_DASHBOARD_TEXT_SCALE_PERCENT
                : view.textScalePercent) / 100.0F;
        textScale = Math.max(MainUiLayoutManager.MIN_DASHBOARD_TEXT_SCALE_PERCENT / 100.0F,
                Math.min(MainUiLayoutManager.MAX_DASHBOARD_TEXT_SCALE_PERCENT / 100.0F, textScale));
        String textAlign = view == null ? MainUiLayoutManager.DASHBOARD_TEXT_ALIGN_CENTER : view.textAlign;
        String textVerticalAlign = view == null ? MainUiLayoutManager.DASHBOARD_TEXT_VERTICAL_CENTER
                : view.textVerticalAlign;
        int badgeSize = 0;
        int badgeX = 0;
        int badgeY = 0;
        if (showIcons) {
            String iconSize = view == null ? MainUiLayoutManager.ICON_MEDIUM : view.iconSize;
            int iconLimit = MainUiLayoutManager.ICON_SMALL.equals(iconSize) ? 28
                    : MainUiLayoutManager.ICON_LARGE.equals(iconSize) || MainUiLayoutManager.ICON_XL.equals(iconSize)
                            ? 58 : 42;
            int heightLimit = prominent ? Math.max(24, bounds.height / 3) : Math.max(20, bounds.height - 12);
            badgeSize = Math.max(20, Math.min(iconLimit, heightLimit));
            badgeX = prominent ? bounds.x + (bounds.width - badgeSize) / 2 : bounds.x + 9;
            badgeY = prominent ? bounds.y + 10 : bounds.y + (bounds.height - badgeSize) / 2;
        ...
        if (prominent) {
            textX = bounds.x + 8;
            textWidth = Math.max(24, bounds.width - 16);
            textTop = showIcons ? badgeY + badgeSize + 7 : bounds.y + 6;
        } else {
            textX = showIcons ? badgeX + badgeSize + 8 : bounds.x + 10;
            textWidth = Math.max(24, bounds.right() - textX - 8);
            textTop = bounds.y + 4;
        }
        textHeight = Math.max(1, bounds.bottom() - (showInfo ? 19 : 5) - textTop);
        drawDashboardCardText(card.title, showInlineInfo ? getCardSubtitle(card, active) : "", showInlineInfo,
                textX, textWidth, textTop, textHeight, textScale, textAlign, textVerticalAlign,
                ModernUiRenderer.readableText(ModernUiRenderer.TEXT, fill),
                ModernUiRenderer.readableText(running ? runningAccent : ModernUiRenderer.SUBTLE_TEXT, fill));
        ...
        if (showInfo) {
            int infoX = bounds.right() - 20;
            int infoY = bounds.bottom() - 18;
    ...
    private void drawDashboardCardText(String title, String subtitle, boolean showSubtitle, int x, int width,
            int top, int height, float scale, String horizontalAlign, String verticalAlign, int titleColor,
            int subtitleColor) {
        List<String> titleLines = wrapDashboardText(title, width, scale);
        if (titleLines.isEmpty()) {
            titleLines.add("");
        }
        float subtitleScale = Math.max(0.55F, scale * 0.82F);
        List<String> subtitleLines = showSubtitle ? wrapDashboardText(subtitle, width, subtitleScale)
                : Collections.<String>emptyList();
        int titleLineHeight = dashboardTextLineHeight(scale);
        int subtitleLineHeight = dashboardTextLineHeight(subtitleScale);
        int subtitleGap = subtitleLines.isEmpty() ? 0 : Math.max(2, Math.round(3.0F * scale));
        int totalHeight = titleLines.size() * titleLineHeight
                + (subtitleLines.isEmpty() ? 0 : subtitleGap + subtitleLines.size() * subtitleLineHeight);
        int drawY = alignDashboardTextBlock(top, height, totalHeight, verticalAlign);
        for (String line : titleLines) {
            drawDashboardTextLine(line, x, width, drawY, scale, horizontalAlign, titleColor);
            drawY += titleLineHeight;
        }
        if (!subtitleLines.isEmpty()) {
            drawY += subtitleGap;
            for (String line : subtitleLines) {
                drawDashboardTextLine(line, x, width, drawY, subtitleScale, horizontalAlign, subtitleColor);
                drawY += subtitleLineHeight;
            }
        }
    }

    private List<String> wrapDashboardText(String value, int maxWidth, float scale) {
        List<String> lines = new ArrayList<String>();
        if (fontRenderer == null || maxWidth <= 0) {
            return lines;
        }
        String translated = ModernFormI18n.tr(value == null ? "" : value);
        String plain = TextFormatting.getTextWithoutFormattingCodes(translated == null ? "" : translated);
        int logicalWidth = Math.max(1, Math.round(maxWidth / Math.max(0.01F, scale)));
        String[] paragraphs = plain.replace("\\n", "\n").split("\n", -1);
        for (String paragraph : paragraphs) {
            ...
            for (int offset = 0; offset < paragraph.length();) {
                ...
                String candidate = current.toString() + unit;
                if (current.length() > 0 && fontRenderer.getStringWidth(candidate) > logicalWidth) {
    ...
    private int dashboardTextLineHeight(float scale) {
        return Math.max(1, Math.round((fontRenderer.FONT_HEIGHT + 2) * scale));
    }

    private int alignDashboardTextBlock(int top, int height, int contentHeight, String verticalAlign) {
        int offset;
        if (MainUiLayoutManager.DASHBOARD_TEXT_VERTICAL_BOTTOM.equals(verticalAlign)) {
            offset = height - contentHeight;
        } else if (MainUiLayoutManager.DASHBOARD_TEXT_VERTICAL_TOP.equals(verticalAlign)) {
            offset = 0;
        } else {
            offset = (height - contentHeight) / 2;
        }
        return top + Math.max(0, offset);
    }

    private void drawDashboardTextLine(String line, int x, int width, int y, float scale, String horizontalAlign,
            int color) {
        if (line == null || line.isEmpty()) {
            return;
        }
        int lineWidth = Math.round(fontRenderer.getStringWidth(line) * scale);
        int drawX = x;
        if (MainUiLayoutManager.DASHBOARD_TEXT_ALIGN_RIGHT.equals(horizontalAlign)) {
            drawX = x + Math.max(0, width - lineWidth);
        } else if (MainUiLayoutManager.DASHBOARD_TEXT_ALIGN_CENTER.equals(horizontalAlign)) {
            drawX = x + Math.max(0, (width - lineWidth) / 2);
        }
        GlStateManager.pushMatrix();
        try {
            GlStateManager.translate(drawX, y, 0.0F);
            GlStateManager.scale(scale, scale, 1.0F);
    ...
    private void drawToolsPopover(ModernMainLayout.Layout layout, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < entries.size(); i++) {
            ...
            int toolIconSize = 14;
            int toolIconColor = hovered ? rowText : rowSubtleText;
            ModernUiRenderer.drawIcon(getToolIcon(entry.action), row.x + 7,
                    row.y + (row.height - toolIconSize) / 2, toolIconSize, toolIconColor);
            ModernUiRenderer.drawText(fontRenderer, entry.title, row.x + 28,
                    row.y + (row.height - fontRenderer.FONT_HEIGHT) / 2,
                    hovered ? rowText : rowSubtleText, Math.max(1, row.width - 68));
            int pinSize = 12;
            int pinX = row.right() - 34;
            int pinY = row.y + (row.height - pinSize) / 2;
            ModernMainLayout.Rect pinBounds = new ModernMainLayout.Rect(pinX - 4, row.y + 2, pinSize + 8,
                    Math.max(1, row.height - 4));
            boolean pinHovered = pinBounds.contains(mouseX, mouseY);
            if (pinHovered) {
                ModernUiRenderer.drawRoundedRect(pinX - 2, pinY - 2, pinSize + 4, pinSize + 4, 4,
                        ModernUiRenderer.SURFACE_HOVER);
            }
            boolean pinned = MainUiLayoutManager.isModernToolPinned(entry.action);
            ModernUiRenderer.drawPinIcon(pinX, pinY, pinned,
                    pinned ? (pinHovered ? rowText : ModernUiRenderer.ACCENT)
                            : (pinHovered ? rowText : ModernUiRenderer.MUTED_TEXT));
            ...
            hitTargets.add(new HitTarget(TargetType.TOOL_PIN, pinBounds, entry.action));
        }
    }
    ...
    private void drawCommandPalette(int mouseX, int mouseY) {
        ...
        commandPaletteScrollbar.draw(this.commandPaletteListBounds, this.commandPaletteScrollOffset,
                ...
                mouseX, mouseY, value -> {
                    this.commandPaletteScrollOffset = value;
                    // Keep the selected item inside the manually scrolled viewport. Otherwise
                    // ensureCommandPaletteSelectionVisible() moves the scrollbar back to the top
                    // on the next frame because the old selection is still index zero.
                    this.commandPaletteSelectedIndex = clamp(value, 0,
                            Math.max(0, this.commandPaletteEntries.size() - 1));
                });
    }
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        if (mouseButton == 0 && beginTabStripDrag(mouseX, mouseY)) {
            return;
        }
        ...
        if (mouseButton == 0 && beginTabStripHeightDrag(mouseX, mouseY)) {
            return;
        }
        ...
        if (mouseButton == 0 && dashboardSettingsScrollbar.beginDrag(mouseX, mouseY)) {
            return;
        }
        if (mouseButton == 0 && beginDashboardSettingsResize(mouseX, mouseY)) {
            return;
        }
        ...
        if (dashboardSettingsOpen && !isDashboardSettingsEmbedded()
                && !isOverDashboardSettingsUi(mouseX, mouseY)) {
        ...
        for (int i = hitTargets.size() - 1; i >= 0; i--) {
            ...
            if (mouseButton == 0 && target.type == TargetType.TAB_SELECT) {
                beginTabReorder(TabKey.decode(target.value), mouseX, mouseY);
            }
    ...
    private boolean handleToolsPopoverClick(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        for (int i = hitTargets.size() - 1; i >= 0; i--) {
            ...
            if ((target.type == TargetType.UTILITY || target.type == TargetType.TOOLS
                    || target.type == TargetType.TOOL_PIN)
    ...
    private boolean beginDashboardSettingsResize(int mouseX, int mouseY) {
        if (!dashboardSettingsOpen || dashboardSettingsResizeBounds == null
                || !dashboardSettingsResizeBounds.contains(mouseX, mouseY)) {
            return false;
        }
        draggingDashboardSettingsResize = true;
        dashboardSettingsResizeStartMouseX = mouseX;
        dashboardSettingsResizeStartWidth = getEffectiveDashboardSettingsWidth();
        dashboardSettingsResizePreviewWidth = dashboardSettingsResizeStartWidth;
        return true;
    }

    private void updateDashboardSettingsResize(int mouseX) {
        if (!draggingDashboardSettingsResize) {
            return;
        }
        int delta = mouseX - dashboardSettingsResizeStartMouseX;
        if (MainUiLayoutManager.DASHBOARD_SETTINGS_RIGHT.equals(getDashboardSettingsPlacement())) {
            delta = -delta;
        }
        int maxWidth = lastTabStrip == null ? MainUiLayoutManager.MAX_DASHBOARD_SETTINGS_WIDTH
                : Math.max(1, lastTabStrip.body.width - 84);
        int minWidth = Math.min(MainUiLayoutManager.MIN_DASHBOARD_SETTINGS_WIDTH, maxWidth);
        dashboardSettingsResizePreviewWidth = clamp(dashboardSettingsResizeStartWidth + delta, minWidth, maxWidth);
    }

    private void finishDashboardSettingsResize() {
        if (!draggingDashboardSettingsResize) {
            return;
        }
        draggingDashboardSettingsResize = false;
        MainUiLayoutManager.setModernDashboardSettingsWidth(dashboardSettingsResizePreviewWidth);
    }
    ...
    private void closeDashboardSettings() {
        ...
        dashboardSettingsAnimating = false;
        draggingDashboardSettingsResize = false;
        dashboardSettingsScrollbar.endDrag();
        dashboardSettingsPanelBounds = null;
        dashboardSettingsViewportBounds = null;
        dashboardSettingsResizeBounds = null;
        dashboardSettingsSubmenuBounds = null;
        dashboardSettingsHoverTool = null;
        dashboardSettingsScroll = 0;
        dashboardSettingsMaxScroll = 0;
    }
    ...
    protected void mouseClickMove(int mouseX, int mouseY, int clickedMouseButton, long timeSinceLastClick) {
        ...
        if (clickedMouseButton == 0 && draggingTabStrip) {
            resizeTabStrip(mouseX);
            return;
        }
        if (clickedMouseButton == 0 && draggingTabStripHeight) {
            resizeTabStripHeight(mouseY);
            return;
        }
        if (clickedMouseButton == 0 && tabReorderPending) {
            updateTabReorder(mouseX, mouseY);
            return;
        }
        ...
        if (clickedMouseButton == 0 && dashboardSettingsScrollbar.isDragging()) {
            dashboardSettingsScrollbar.applyDrag(mouseX, mouseY);
            return;
        }
        if (clickedMouseButton == 0 && draggingDashboardSettingsResize) {
            updateDashboardSettingsResize(mouseX);
            return;
        }
    ...
    protected void mouseReleased(int mouseX, int mouseY, int state) {
        ...
        boolean wasDraggingTabStrip = state == 0 && draggingTabStrip;
        boolean wasDraggingTabStripHeight = state == 0 && draggingTabStripHeight;
        ...
        boolean wasDraggingDashboardSettingsScrollbar = state == 0 && dashboardSettingsScrollbar.isDragging();
        boolean wasResizingDashboardSettings = state == 0 && draggingDashboardSettingsResize;
        if (state == 0) {
            resizeHandle = ResizeHandle.NONE;
            draggingWindow = false;
            draggingSidebar = false;
            draggingTabStrip = false;
            draggingTabStripHeight = false;
            draggingTabScrollbar = false;
            draggingNavigationScrollbar = false;
            horizontalTabScrollbar.endDrag();
            verticalTabScrollbar.endDrag();
            navigationScrollbar.endDrag();
            dashboardScrollbar.endDrag();
            dashboardSettingsScrollbar.endDrag();
        }
        if (wasResizingDashboardSettings) {
            finishDashboardSettingsResize();
        }
        if (wasResizing || wasMoving || wasDraggingSidebar || wasDraggingTabStrip || wasDraggingTabStripHeight
                || wasDraggingTabScrollbar
                || wasDraggingNavigationScrollbar || wasDraggingDashboardScrollbar
                || wasDraggingDashboardSettingsScrollbar || wasResizingDashboardSettings) {
            if (wasDraggingSidebar) {
                MainUiLayoutManager.setModernSidebarWidth(sidebarWidth);
            }
            if (wasDraggingTabStrip) {
                MainUiLayoutManager.setModernTabStripWidth(tabStripInnerWidth);
            }
            if (wasDraggingTabStripHeight) {
                MainUiLayoutManager.setModernTabStripHeight(tabStripHeight);
            }
            persistWindowLayout();
            ModernTooltipSupport.setSuppressed(this, false);
            return;
    ...
    private void openKeybindSettingsForEntry(CommandPaletteEntry entry) {
        ...
        if (keybindTab != null) {
            ...
            if (restorePageStateAfterFirstDraw.contains(keybindTab)) {
                pendingTabFocusAfterStateRestore.put(keybindTab, entry.keybindTarget);
            }
        }
    }
        ...
        switch (target.type) {
            ...
            case HEADER_TOGGLE:
                if (mouseButton == 0) {
                    ...
                    MainUiLayoutManager.setModernHeaderCollapsed(headerCollapsed);
                    toolsOpen = false;
                    closeCommandPalette();
                }
                return;
            ...
            case TOOL_PIN:
                if (mouseButton == 0 && target.value != null) {
                    MainUiLayoutManager.toggleModernToolPinned(target.value);
                }
                return;
            case DETACH:
                if (mouseButton == 0) {
            ...
            case TAB_SELECT:
                if (mouseButton == 0) {
                    ...
                    openTabItemContextMenu(mouseX, mouseY, TabKey.decode(target.value));
                }
                return;
            case TAB_CLOSE:
                if (mouseButton == 0) {
            ...
            case TAB_STRIP:
                if (mouseButton == 1) {
                    openTabStripContextMenu(mouseX, mouseY);
                }
                return;
            case INFO:
                return;
    ...
    private void handleDashboardTool(String tool, int mouseButton) {
        ...
        if ("placement".equals(tool)) {
            String[] values = { MainUiLayoutManager.DASHBOARD_SETTINGS_FLOATING,
                    MainUiLayoutManager.DASHBOARD_SETTINGS_LEFT, MainUiLayoutManager.DASHBOARD_SETTINGS_RIGHT };
            MainUiLayoutManager.setModernDashboardSettingsPlacement(
                    cycleValue(values, getDashboardSettingsPlacement(), direction));
        } else if ("panel_width".equals(tool)) {
            String[] values = toStringArray(DASHBOARD_SETTINGS_WIDTH_OPTIONS);
            MainUiLayoutManager.setModernDashboardSettingsWidth(Integer.parseInt(
                    cycleValue(values, String.valueOf(getEffectiveDashboardSettingsWidth()), direction)));
        } else if ("sort".equals(tool)) {
            ...
            MainUiLayoutManager.setModernDashboardShowIcons(viewKey, !view.showIcons);
        } else if ("size".equals(tool)) {
            ...
            String[] values = toStringArray(DASHBOARD_CARD_WIDTH_OPTIONS);
            MainUiLayoutManager.setModernDashboardCardWidth(viewKey,
                    Integer.parseInt(cycleValue(values, String.valueOf(view.cardWidth), direction)));
        } else if ("height".equals(tool)) {
            String[] values = toStringArray(DASHBOARD_CARD_HEIGHT_OPTIONS);
            MainUiLayoutManager.setModernDashboardCardHeight(viewKey,
                    Integer.parseInt(cycleValue(values, String.valueOf(view.cardHeight), direction)));
        } else if ("text_size".equals(tool)) {
            String[] values = toStringArray(DASHBOARD_TEXT_SCALE_OPTIONS);
            MainUiLayoutManager.setModernDashboardTextScalePercent(viewKey,
                    Integer.parseInt(cycleValue(values, String.valueOf(view.textScalePercent), direction)));
        } else if ("text_align".equals(tool)) {
            String[] values = { MainUiLayoutManager.DASHBOARD_TEXT_ALIGN_LEFT,
                    MainUiLayoutManager.DASHBOARD_TEXT_ALIGN_CENTER, MainUiLayoutManager.DASHBOARD_TEXT_ALIGN_RIGHT };
            MainUiLayoutManager.setModernDashboardTextAlign(viewKey,
                    cycleValue(values, view.textAlign, direction));
        } else if ("text_vertical".equals(tool)) {
            String[] values = { MainUiLayoutManager.DASHBOARD_TEXT_VERTICAL_TOP,
                    MainUiLayoutManager.DASHBOARD_TEXT_VERTICAL_CENTER,
                    MainUiLayoutManager.DASHBOARD_TEXT_VERTICAL_BOTTOM };
            MainUiLayoutManager.setModernDashboardTextVerticalAlign(viewKey,
                    cycleValue(values, view.textVerticalAlign, direction));
        } else if ("info".equals(tool)) {
            String[] values = { MainUiLayoutManager.DASHBOARD_INFO_ICON,
                    MainUiLayoutManager.DASHBOARD_INFO_INLINE, MainUiLayoutManager.DASHBOARD_INFO_HIDDEN };
            MainUiLayoutManager.setModernDashboardInfoMode(viewKey,
                    cycleValue(values, view.infoMode, direction));
        } else if ("columns".equals(tool)) {
    ...
    private void handleDashboardToolOption(String encoded, int mouseButton) {
        ...
        String viewKey = getDashboardViewKey();
        if ("panel_width".equals(tool) && "custom".equals(optionId)) {
            openInlineTextInput("gui.modern.dashboard.settings_width_custom",
                    String.valueOf(getEffectiveDashboardSettingsWidth()),
                    value -> applyCustomDashboardSettingsWidth(value));
            return;
        }
        if (("width".equals(tool) || "height".equals(tool) || "text_size".equals(tool))
                && "custom".equals(optionId)) {
            MainUiLayoutManager.DashboardViewMeta currentView = getDashboardView();
            int current;
            String title;
            if ("width".equals(tool)) {
                current = currentView.cardWidth;
                title = "gui.modern.dashboard.card_width_custom";
            } else if ("height".equals(tool)) {
                current = currentView.cardHeight;
                title = "gui.modern.dashboard.card_height_custom";
            } else {
                current = currentView.textScalePercent;
                title = "gui.modern.dashboard.text_size_custom";
            }
            String initial = current > 0 ? String.valueOf(current)
                    : "width".equals(tool) ? "220" : "text_size".equals(tool) ? "100" : "96";
            openInlineTextInput(title, initial,
                    value -> applyCustomDashboardDimension(viewKey, tool, value));
            return;
        }
        if ("placement".equals(tool)) {
            MainUiLayoutManager.setModernDashboardSettingsPlacement(optionId);
        } else if ("panel_width".equals(tool)) {
            try {
                MainUiLayoutManager.setModernDashboardSettingsWidth(Integer.parseInt(optionId));
            } catch (NumberFormatException ignored) {
                return;
            }
        } else if ("sort".equals(tool)) {
            ...
            MainUiLayoutManager.setModernDashboardShowIcons(viewKey, "on".equals(optionId));
        } else if ("size".equals(tool)) {
            MainUiLayoutManager.setModernDashboardIconSize(viewKey, optionId);
        } else if ("width".equals(tool)) {
            try {
                MainUiLayoutManager.setModernDashboardCardWidth(viewKey, Integer.parseInt(optionId));
            } catch (NumberFormatException ignored) {
                return;
            }
        } else if ("height".equals(tool)) {
            try {
                MainUiLayoutManager.setModernDashboardCardHeight(viewKey, Integer.parseInt(optionId));
            } catch (NumberFormatException ignored) {
                return;
            }
        } else if ("text_size".equals(tool)) {
            try {
                MainUiLayoutManager.setModernDashboardTextScalePercent(viewKey, Integer.parseInt(optionId));
            } catch (NumberFormatException ignored) {
                return;
            }
        } else if ("text_align".equals(tool)) {
            MainUiLayoutManager.setModernDashboardTextAlign(viewKey, optionId);
        } else if ("text_vertical".equals(tool)) {
            MainUiLayoutManager.setModernDashboardTextVerticalAlign(viewKey, optionId);
        } else if ("info".equals(tool)) {
            MainUiLayoutManager.setModernDashboardInfoMode(viewKey, optionId);
        } else if ("columns".equals(tool)) {
            try {
    ...
    private void applyCustomDashboardDimension(String viewKey, String tool, String value) {
        try {
            int dimension = Integer.parseInt(value == null ? "" : value.trim());
            if ("width".equals(tool)) {
                MainUiLayoutManager.setModernDashboardCardWidth(viewKey, dimension);
            } else if ("height".equals(tool)) {
                MainUiLayoutManager.setModernDashboardCardHeight(viewKey, dimension);
            } else if ("text_size".equals(tool)) {
                MainUiLayoutManager.setModernDashboardTextScalePercent(viewKey, dimension);
            }
            resetDashboardPosition();
        } catch (NumberFormatException ignored) {
            // Keep the previous dimension when the inline value is not numeric.
        }
    }

    private void applyCustomDashboardSettingsWidth(String value) {
        try {
            MainUiLayoutManager.setModernDashboardSettingsWidth(Integer.parseInt(value == null ? "" : value.trim()));
        } catch (NumberFormatException ignored) {
            // Keep the previous panel width when the inline value is not numeric.
        }
    }

    private String[] toStringArray(int[] values) {
    ...
    private void openTabStripContextMenu(int mouseX, int mouseY) {
        List<GuiInventoryBase.ContextMenuItem> items = new ArrayList<>();
        items.add(orientationMenuItem());
        items.add(GuiInventoryBase.ContextMenuItem.separator());
        items.add(restoreClosedTabMenuItem());
        items.add(closeActiveTabMenuItem());
        items.add(closeAllTabsMenuItem());
        openModernContextMenu(mouseX, mouseY, items);
    }

    private void openTabItemContextMenu(int mouseX, int mouseY, TabKey key) {
        List<GuiInventoryBase.ContextMenuItem> items = new ArrayList<>();
        items.add(closeTabMenuItem(key));
        items.add(closeOtherTabsMenuItem(key));
        items.add(closeAllTabsMenuItem());
        items.add(GuiInventoryBase.ContextMenuItem.separator());
        items.add(restoreClosedTabMenuItem());
        items.add(GuiInventoryBase.ContextMenuItem.separator());
        items.add(toggleTabFullscreenMenuItem(key));
        items.add(GuiInventoryBase.ContextMenuItem.separator());
        items.add(orientationMenuItem());
        openModernContextMenu(mouseX, mouseY, items);
    }

    private GuiInventoryBase.ContextMenuItem toggleTabFullscreenMenuItem(TabKey key) {
        return new GuiInventoryBase.ContextMenuItem(
                isTabFullscreen(key) ? "gui.modern.main.u144" : "gui.modern.main.u143",
                () -> toggleTabFullscreen(key))
                .shortcut(KeybindManager.menuShortcut(BindableAction.TOGGLE_TAB_FULLSCREEN))
                .enabled(key != null && !key.isDashboard() && !DetachedSwingWindowManager.isDetached());
    }

    private GuiInventoryBase.ContextMenuItem orientationMenuItem() {
        boolean vertical = MainUiLayoutManager.isModernTabStripVertical();
        return new GuiInventoryBase.ContextMenuItem(
                vertical ? "gui.modern.main.u134" : "gui.modern.main.u133", this::toggleTabStripOrientation);
    }

    private GuiInventoryBase.ContextMenuItem restoreClosedTabMenuItem() {
        return new GuiInventoryBase.ContextMenuItem("gui.modern.main.u136", this::restoreClosedTab)
                .shortcut(KeybindManager.menuShortcut(BindableAction.RESTORE_CLOSED_TAB))
                .enabled(!closedTabs.isEmpty());
    }

    private GuiInventoryBase.ContextMenuItem closeActiveTabMenuItem() {
        return new GuiInventoryBase.ContextMenuItem("gui.modern.main.u139", this::closeActiveTab)
                .shortcut(KeybindManager.menuShortcut(BindableAction.CLOSE_ACTIVE_TAB))
                .enabled(activeTab != null && !activeTab.isDashboard());
    }

    private GuiInventoryBase.ContextMenuItem closeTabMenuItem(TabKey key) {
        TabKey target = key;
        return new GuiInventoryBase.ContextMenuItem("gui.modern.main.u139", () -> requestCloseTab(target))
                .shortcut(KeybindManager.menuShortcut(BindableAction.CLOSE_ACTIVE_TAB))
                .enabled(target != null && !target.isDashboard());
    }

    private GuiInventoryBase.ContextMenuItem closeOtherTabsMenuItem(TabKey key) {
        TabKey target = key;
        return new GuiInventoryBase.ContextMenuItem("gui.modern.main.u138", () -> requestCloseOtherTabs(target))
                .enabled(hasOtherClosableTabs(target));
    }

    private GuiInventoryBase.ContextMenuItem closeAllTabsMenuItem() {
        return new GuiInventoryBase.ContextMenuItem("gui.modern.main.u137", this::closeAllTabs)
                .shortcut(KeybindManager.menuShortcut(BindableAction.CLOSE_ALL_TABS))
                .enabled(hasClosableTabs());
    }

    private void toggleTabStripOrientation() {
        MainUiLayoutManager.setModernTabStripVertical(!MainUiLayoutManager.isModernTabStripVertical());
        tabScrollOffset = 0;
    ...
    private List<List<TabKey>> tabGroups() {
        return ModernTabOrder.grouped(openTabs, new ModernTabOrder.RootOf<TabKey>() {
            @Override
            public Object of(TabKey item) {
                TabKey root = dependencyRoot(item);
                return root == null ? item : root;
            }
        });
    }

    private int pinnedTabGroupCount(List<List<TabKey>> groups) {
        if (groups != null && !groups.isEmpty() && groups.get(0).size() == 1 && groups.get(0).get(0).isDashboard()) {
            return 1;
        }
        return 0;
    }

    private boolean beginTabReorder(TabKey key, int mouseX, int mouseY) {
        if (key == null || lastTabStrip == null) {
            return false;
        }
        List<List<TabKey>> groups = tabGroups();
        int from = ModernTabOrder.groupIndexOf(groups, key);
        int minInsert = pinnedTabGroupCount(groups);
        if (from < minInsert) {
            return false;
        }
        tabReorderPending = true;
        tabReorderActive = false;
        tabReorderKey = key;
        tabReorderStartX = mouseX;
        tabReorderStartY = mouseY;
        tabReorderDropInsertBefore = from;
        return true;
    }

    private void updateTabReorder(int mouseX, int mouseY) {
        if (!tabReorderPending) {
            return;
        }
        if (!tabReorderActive) {
            int deltaX = mouseX - tabReorderStartX;
            int deltaY = mouseY - tabReorderStartY;
            if (deltaX * deltaX + deltaY * deltaY < PATH_DRAG_START_DISTANCE * PATH_DRAG_START_DISTANCE) {
                return;
            }
            tabReorderActive = true;
            ModernTooltipSupport.setSuppressed(this, true);
        }
        List<List<TabKey>> groups = tabGroups();
        int minInsert = pinnedTabGroupCount(groups);
        int[] mids = new int[groups.size()];
        boolean vertical = lastTabStrip != null && lastTabStrip.vertical();
        for (int i = 0; i < groups.size(); i++) {
            ModernMainLayout.Rect bounds = unionTabBounds(groups.get(i));
            if (bounds == null) {
                mids[i] = vertical ? mouseY : mouseX;
            } else {
                mids[i] = vertical ? bounds.y + bounds.height / 2 : bounds.x + bounds.width / 2;
            }
        }
        tabReorderDropInsertBefore = ModernTabOrder.insertBeforeFromPointer(mids, vertical ? mouseY : mouseX, minInsert);
    }

    private void finishTabReorder() {
        if (tabReorderActive && tabReorderKey != null) {
            List<List<TabKey>> groups = tabGroups();
            int from = ModernTabOrder.groupIndexOf(groups, tabReorderKey);
            if (ModernTabOrder.moveGroup(groups, from, tabReorderDropInsertBefore)) {
                openTabs.clear();
                openTabs.addAll(ModernTabOrder.flatten(groups));
                ensureActiveTabVisible();
            }
        }
        clearTabReorder();
    ...
    private void clearTabReorder() {
        tabReorderPending = false;
        tabReorderActive = false;
        tabReorderKey = null;
        tabReorderDropInsertBefore = 0;
        ModernTooltipSupport.setSuppressed(this, false);
    }

    private ModernMainLayout.Rect unionTabBounds(List<TabKey> group) {
        if (group == null || group.isEmpty()) {
            return null;
        }
        ModernMainLayout.Rect first = lastRenderedTabBounds.get(group.get(0));
        if (first == null) {
            return null;
        }
        int left = first.x;
        ...
        for (int i = 1; i < group.size(); i++) {
            ModernMainLayout.Rect bounds = lastRenderedTabBounds.get(group.get(i));
            if (bounds == null) {
                continue;
            }
        ...
        return new ModernMainLayout.Rect(left, top, right - left, bottom - top);
    }

    private void drawTabReorderDropLine(ModernTabStripLayout.Layout stripLayout) {
        if (!tabReorderActive || stripLayout == null || tabReorderKey == null) {
            return;
        }
        List<List<TabKey>> groups = tabGroups();
        int from = ModernTabOrder.groupIndexOf(groups, tabReorderKey);
        int insertBefore = tabReorderDropInsertBefore;
        if (from < 0 || insertBefore == from || insertBefore == from + 1) {
            return;
        }
        boolean vertical = stripLayout.vertical();
        int lineX;
        int lineY;
        int lineW;
        int lineH;
        if (insertBefore <= 0) {
            ModernMainLayout.Rect first = unionTabBounds(groups.get(0));
            if (first == null) {
                return;
            }
            lineX = first.x;
            lineY = first.y;
            lineW = vertical ? first.width : 2;
            lineH = vertical ? 2 : first.height;
        } else if (insertBefore >= groups.size()) {
            ModernMainLayout.Rect last = unionTabBounds(groups.get(groups.size() - 1));
            if (last == null) {
                return;
            }
            lineX = vertical ? last.x : last.right() - 2;
            lineY = vertical ? last.bottom() - 2 : last.y;
            lineW = vertical ? last.width : 2;
            lineH = vertical ? 2 : last.height;
        } else {
            ModernMainLayout.Rect next = unionTabBounds(groups.get(insertBefore));
            if (next == null) {
                return;
            }
            lineX = vertical ? next.x : next.x - 1;
            lineY = vertical ? next.y - 1 : next.y;
            lineW = vertical ? next.width : 2;
            lineH = vertical ? 2 : next.height;
        }
        ModernUiRenderer.drawRoundedRect(lineX, lineY, Math.max(2, lineW), Math.max(2, lineH), 1,
                ModernUiRenderer.ACCENT);
    }

    private void drawTabReorderGhost(int mouseX, int mouseY) {
        if (!tabReorderActive || tabReorderKey == null) {
            return;
        }
        List<List<TabKey>> groups = tabGroups();
        ...
        int dx = mouseX - tabReorderStartX;
        int dy = mouseY - tabReorderStartY;
        List<TabKey> group = groups.get(from);
        for (int i = 0; i < group.size(); i++) {
            TabKey tab = group.get(i);
            ModernMainLayout.Rect bounds = lastRenderedTabBounds.get(tab);
            if (bounds == null) {
                continue;
            }
            int x = bounds.x + dx;
            int y = bounds.y + dy;
            ModernUiRenderer.drawSubtlePanel(x, y, bounds.width, bounds.height, 5,
                    ModernUiRenderer.SELECTED_SURFACE, ModernUiRenderer.ACCENT);
            int titleWidth = bounds.width - (tab.isDashboard() ? ModernTabStripLayout.DASHBOARD_CHROME
                    : ModernTabStripLayout.CLOSE_CHROME);
            ModernUiRenderer.drawText(fontRenderer, getTabTitle(tab), x + ModernTabStripLayout.TITLE_INSET,
                    y + (bounds.height - fontRenderer.FONT_HEIGHT) / 2, ModernUiRenderer.TEXT,
                    Math.max(1, titleWidth));
        }
    }
    ...
    public void handleDetachedMouseWheel(int wheel, int mouseX, int mouseY) {
        ...
        if (dashboardSettingsOpen && dashboardSettingsViewportBounds != null
                && dashboardSettingsViewportBounds.contains(mouseX, mouseY)
                && dashboardSettingsMaxScroll > 0) {
            int amount = wheel > 0 ? -38 : 38;
            dashboardSettingsScroll = clamp(dashboardSettingsScroll + amount, 0, dashboardSettingsMaxScroll);
            return;
        }
        ModernSettingsTab settingsTab = getActiveSettingsTab();
    ...
    private void openSettingsTabForCommand(String command) {
        if ("reset_window".equals(command)) {
            resetWindowLayout();
            return;
        }
        ...
        try {
            ...
            boolean restorePageState = tab == null;
            if (tab == null) {
            ...
            tab.ensureInitialized(this.fontRenderer);
            if (restorePageState) {
                restorePageState(stateCommandForTab(key), tab);
                restorePageStateAfterFirstDraw.add(tab);
            }
            activateTab(key);
            ensureActiveTabVisible();
            toolsOpen = false;
            ...
            // Header actions are navigation actions too. Their tab creation and
            // nested routes must not turn an already-collapsed main header back
            // into the expanded layout.
            if (headerWasCollapsed) {
    ...
    private void closeTab(TabKey key) {
        ...
        if (isTabFullscreen(key)) {
            fullscreenTab = null;
        }
        capturePageState(stateCommandForTab(key), settingsTabs.get(key), true);
        flushPageStates();
        recordClosedTab(key);
    ...
    private void requestCloseTab(TabKey key) {
        ...
        if (hasDirtyTabInCascade(key)) {
            openCloseConfirmation(key, cascade, CloseConfirmKind.TAB);
            return;
        }
        if (cascade) {
            closeTabCascade(key);
    ...
    private void requestCloseAllTabs() {
        List<TabKey> closing = closableTabs(null);
        if (closing.isEmpty()) {
            return;
        }
        if (hasDirtyTabs(closing)) {
            openCloseConfirmation(activeTab, false, CloseConfirmKind.ALL);
            return;
        }
        closeTabs(closing);
    }

    private void requestCloseOtherTabs(TabKey keep) {
        List<TabKey> closing = closableTabs(keep);
        if (closing.isEmpty()) {
            return;
        }
        if (hasDirtyTabs(closing)) {
            openCloseConfirmation(keep, false, CloseConfirmKind.OTHERS);
            return;
        }
        closeTabs(closing);
    }
    ...
    private boolean hasClosableTabs() {
        return !closableTabs(null).isEmpty();
    }

    private boolean hasOtherClosableTabs(TabKey keep) {
        return !closableTabs(keep).isEmpty();
    }

    private List<TabKey> closableTabs(TabKey keep) {
        List<TabKey> closing = new ArrayList<>();
        TabKey keepRoot = keep == null ? null : dependencyRoot(keep);
        for (int i = 0; i < openTabs.size(); i++) {
            TabKey candidate = openTabs.get(i);
            if (candidate.isDashboard()) {
                continue;
            }
            if (keepRoot != null) {
                TabKey candidateRoot = dependencyRoot(candidate);
                if (keepRoot.equals(candidateRoot) || keepRoot.equals(candidate)) {
                    continue;
                }
            }
            closing.add(candidate);
    ...
    private boolean hasDirtyTabs(List<TabKey> keys) {
        if (keys == null) {
            return false;
        }
        for (int i = 0; i < keys.size(); i++) {
            ModernSettingsTab tab = settingsTabs.get(keys.get(i));
    ...
    private void closeTabs(List<TabKey> keys) {
    ...
    private void saveTabs(List<TabKey> keys) {
        if (keys == null) {
            return;
        }
        Set<ModernSettingsTab> saved = Collections.newSetFromMap(new IdentityHashMap<ModernSettingsTab, Boolean>());
        for (int i = 0; i < keys.size(); i++) {
            ModernSettingsTab tab = settingsTabs.get(keys.get(i));
            if (tab != null && saved.add(tab)) {
                tab.save();
            }
        }
    }

    private List<TabKey> tabsForCloseConfirmation() {
        if (closeConfirmationKind == CloseConfirmKind.ALL) {
            return closableTabs(null);
        }
        if (closeConfirmationKind == CloseConfirmKind.OTHERS) {
            return closableTabs(closeConfirmationTab);
        }
        List<TabKey> closing = new ArrayList<>();
        TabKey root = closeConfirmationTab;
        if (root == null) {
            return closing;
        }
        for (int i = 0; i < openTabs.size(); i++) {
    ...
    private void openCloseConfirmation(TabKey key, boolean cascade, CloseConfirmKind kind) {
        closeConfirmationTab = key;
        closeConfirmationCascade = cascade;
        closeConfirmationKind = kind == null ? CloseConfirmKind.TAB : kind;
    ...
    private void closeCloseConfirmation() {
        ...
        closeConfirmationKind = CloseConfirmKind.TAB;
        pendingSettingsReplacementCommand = "";
        closeConfirmationBounds = null;
        closeConfirmationConfirmBounds = null;
        closeConfirmationSaveBounds = null;
        closeConfirmationCancelBounds = null;
    }

    private void confirmCloseConfirmation() {
        applyCloseConfirmation(false);
    }

    private void saveAndCloseConfirmation() {
        applyCloseConfirmation(true);
    }

    private void applyCloseConfirmation(boolean save) {
        ...
        CloseConfirmKind kind = closeConfirmationKind;
        List<TabKey> closing = tabsForCloseConfirmation();
        closeCloseConfirmation();
        if (save) {
            saveTabs(closing);
        }
        if (kind == CloseConfirmKind.ALL || kind == CloseConfirmKind.OTHERS) {
            closeTabs(closing);
            return;
        }
        if (key == null) {
    ...
    private boolean handleCloseConfirmationClick(int mouseX, int mouseY, int mouseButton) {
        ...
        if (closeConfirmationSaveBounds != null && closeConfirmationSaveBounds.contains(mouseX, mouseY)) {
            saveAndCloseConfirmation();
            return true;
        }
    ...
    private void drawCloseConfirmation(int mouseX, int mouseY) {
        ModernMainLayout.Rect shell = lastLayout == null
                ? new ModernMainLayout.Rect(0, 0, Math.max(1, width), Math.max(1, height))
                : lastLayout.shell;
        int panelWidth = Math.min(420, Math.max(340, shell.width - 20));
        int panelHeight = Math.min(148, Math.max(126, shell.height - 20));
        ...
        int buttonGap = 8;
        int buttonWidth = Math.max(96, (panelWidth - 28 - buttonGap * 2) / 3);
        int buttonY = y + panelHeight - 31;
        closeConfirmationConfirmBounds = new ModernMainLayout.Rect(x + 14, buttonY, buttonWidth, 22);
        closeConfirmationSaveBounds = new ModernMainLayout.Rect(x + 14 + buttonWidth + buttonGap, buttonY,
                buttonWidth, 22);
        closeConfirmationCancelBounds = new ModernMainLayout.Rect(x + panelWidth - 14 - buttonWidth, buttonY,
                buttonWidth, 22);

        // Keep the modal surface neutral so the warning color is reserved for
        // the destructive action. The old warning border made the whole panel
        // look like a fixed brown overlay with some themes.
        ModernUiRenderer.drawBackdropOverlay(this.width, this.height, 0x88000000);
        ModernUiRenderer.drawPanel(x, y, panelWidth, panelHeight, 7, ModernUiRenderer.SHELL_RAISED,
                ModernUiRenderer.BORDER);
        String title = closeConfirmationKind == CloseConfirmKind.ALL
                ? "gui.modern.main.u137"
                : closeConfirmationKind == CloseConfirmKind.OTHERS
                        ? "gui.modern.main.u138"
                        : closeConfirmationTab == null ? "gui.modern.main.u076" : "gui.modern.main.u077";
        ModernUiRenderer.drawText(fontRenderer, title, x + 16, y + 12, ModernUiRenderer.TEXT, panelWidth - 32);
        String message = closeConfirmationKind == CloseConfirmKind.ALL
                ? "gui.modern.main.u141"
                : closeConfirmationKind == CloseConfirmKind.OTHERS
                        ? "gui.modern.main.u142"
                        : closeConfirmationTab == null
        ...
        ModernUiRenderer.drawText(fontRenderer, message, x + 16, y + 44, ModernUiRenderer.SUBTLE_TEXT,
                panelWidth - 32);
        drawCloseConfirmationButton(closeConfirmationConfirmBounds, "gui.modern.main.u081", ConfirmButtonStyle.DANGER,
                mouseX, mouseY);
        drawCloseConfirmationButton(closeConfirmationSaveBounds, "gui.modern.main.u140", ConfirmButtonStyle.ACCENT,
                mouseX, mouseY);
        drawCloseConfirmationButton(closeConfirmationCancelBounds, "gui.modern.main.u082", ConfirmButtonStyle.NEUTRAL,
                mouseX, mouseY);
    }

    private void drawCloseConfirmationButton(ModernMainLayout.Rect bounds, String label, boolean danger,
            int mouseX, int mouseY) {
        drawCloseConfirmationButton(bounds, label, danger ? ConfirmButtonStyle.DANGER : ConfirmButtonStyle.NEUTRAL,
                mouseX, mouseY);
    }

    private void drawCloseConfirmationButton(ModernMainLayout.Rect bounds, String label, ConfirmButtonStyle style,
            int mouseX, int mouseY) {
        if (bounds == null) {
            return;
        }
        boolean hovered = bounds.contains(mouseX, mouseY);
        ConfirmButtonStyle effective = style == null ? ConfirmButtonStyle.NEUTRAL : style;
        int fill;
        int border;
        int textColor;
        if (effective == ConfirmButtonStyle.DANGER) {
            fill = hovered
                    ? ModernUiRenderer.DANGER
                    : ModernUiRenderer.lerpRgb(ModernUiRenderer.SHELL_RAISED, ModernUiRenderer.DANGER, 0.42F);
            border = ModernUiRenderer.DANGER;
            textColor = ModernUiRenderer.readableText(ModernUiRenderer.TEXT, fill);
        } else if (effective == ConfirmButtonStyle.ACCENT) {
            fill = hovered ? ModernUiRenderer.ACCENT : ModernUiRenderer.ACCENT_DIM;
        ...
        com.zszl.zszlScriptMod.gui.modern.SaveShortcutHint.drawText(fontRenderer, label, bounds.x + 8,
                bounds.y + (bounds.height - fontRenderer.FONT_HEIGHT) / 2, textColor, bounds.width - 16);
    }
    ...
    public void selectTabSlot(int slot) {
        ...
        activateTab(openTabs.get(slot - 1));
        ensureActiveTabVisible();
    }
    ...
    private boolean beginTabStripDrag(int mouseX, int mouseY) {
        if (lastTabStrip == null || !lastTabStrip.vertical()
                || tabStripDividerBounds == null || !tabStripDividerBounds.contains(mouseX, mouseY)) {
            return false;
        }
        long now = System.currentTimeMillis();
        if (now - lastTabStripDividerClickAt <= 350L && Math.abs(mouseX - lastTabStripDividerClickX) <= 4) {
            tabStripInnerWidth = -1;
            MainUiLayoutManager.setModernTabStripWidth(-1);
            lastTabStripDividerClickAt = 0L;
            return true;
        }
        lastTabStripDividerClickAt = now;
        lastTabStripDividerClickX = mouseX;
        draggingTabStrip = true;
        tabStripDragStartWidth = lastTabStrip.innerWidth;
        tabStripInnerWidth = lastTabStrip.innerWidth;
        ModernTooltipSupport.setSuppressed(this, true);
        return true;
    }

    private void resizeTabStrip(int mouseX) {
        if (lastTabStrip == null || lastLayout == null) {
            return;
        }
        int minWidth = currentTabStripMinWidth();
        int maxWidth = ModernTabStripLayout.maxVerticalInnerWidth(lastLayout.content);
        int inner = mouseX - lastTabStrip.strip.x - ModernHoverScrollbar.GUTTER;
        tabStripInnerWidth = clamp(inner, minWidth, Math.max(minWidth, maxWidth));
    }

    private boolean beginTabStripHeightDrag(int mouseX, int mouseY) {
        if (lastTabStrip == null || lastTabStrip.vertical() || tabStripDividerBounds == null
                || !tabStripDividerBounds.contains(mouseX, mouseY)) {
            return false;
        }
        long now = System.currentTimeMillis();
        if (now - lastTabStripHeightDividerClickAt <= 350L
                && Math.abs(mouseY - lastTabStripHeightDividerClickY) <= 4) {
            tabStripHeight = -1;
            MainUiLayoutManager.setModernTabStripHeight(-1);
            lastTabStripHeightDividerClickAt = 0L;
            return true;
        }
        lastTabStripHeightDividerClickAt = now;
        lastTabStripHeightDividerClickY = mouseY;
        draggingTabStripHeight = true;
        tabStripDragStartHeight = lastTabStrip.tabHeight;
        tabStripDragStartMouseY = mouseY;
        tabStripHeight = lastTabStrip.tabHeight;
        ModernTooltipSupport.setSuppressed(this, true);
        return true;
    }

    private void resizeTabStripHeight(int mouseY) {
        if (!draggingTabStripHeight || lastLayout == null || lastTabStrip == null) {
            return;
        }
        int minHeight = ModernTabStripLayout.MIN_HORIZONTAL_TAB_HEIGHT;
        int maxHeight = ModernTabStripLayout.maxHorizontalTabHeight(lastLayout.content);
        int height = tabStripDragStartHeight + mouseY - tabStripDragStartMouseY;
        tabStripHeight = clamp(height, Math.min(minHeight, maxHeight), maxHeight);
    }

    private int currentTabStripMinWidth() {
        if (fontRenderer == null) {
            return ModernTabStripLayout.minVerticalInnerWidth(ModernTabStripLayout.DEFAULT_FONT_HEIGHT);
        }
        int fontSize = Math.max(1, fontRenderer.FONT_HEIGHT);
        int glyphWidth = fontRenderer.getStringWidth("中");
        if (glyphWidth > fontSize) {
            fontSize = glyphWidth;
        }
        return ModernTabStripLayout.minVerticalInnerWidth(fontSize);
    }
    ...
    private boolean cancelActiveGesture() {
        if (pathDragPending) {
            ...
            draggingSidebar = false;
        } else if (draggingTabStrip) {
            tabStripInnerWidth = tabStripDragStartWidth;
            draggingTabStrip = false;
        } else if (draggingTabStripHeight) {
            tabStripHeight = tabStripDragStartHeight;
            draggingTabStripHeight = false;
        } else if (tabReorderPending) {
            clearTabReorder();
    ...
    private boolean isGestureActive() {
        return pathDragPending || resizeHandle != ResizeHandle.NONE || draggingWindow || draggingSidebar
                || draggingTabStrip || draggingTabStripHeight || tabReorderActive
    ...
    /** Returns whether the active settings tab currently occupies the menu body. */
    public boolean isActiveTabFullscreen() {
        return fullscreenTab != null && fullscreenTab.equals(activeTab);
    }

    private boolean isTabFullscreen(TabKey key) {
        return fullscreenTab != null && fullscreenTab.equals(key);
    }

    /** Toggles the active settings tab between normal and menu-body layout. */
    public void toggleActiveTabFullscreen() {
        toggleTabFullscreen(activeTab);
    }

    private void toggleTabFullscreen(TabKey key) {
        if (DetachedSwingWindowManager.isDetachedScreen(this)) {
            return;
        }
        if (key == null || key.isDashboard() || !openTabs.contains(key)) {
            return;
        }
        if (!key.equals(activeTab)) {
            activateTab(key);
        ...
        fullscreenTab = isActiveTabFullscreen() ? null : key;
        dashboardSettingsOpen = false;
        dashboardSettingsAnimating = false;
        toolsOpen = false;
        cancelDetachedInteractions();
    ...
    private String stateCommandForTab(TabKey key) {
        if (key == null || key.isDashboard()) {
            return "";
        }
        String command = commandForTabCache(key);
        ...
        return normalizeStateCommand(command);
    }

    private String normalizeStateCommand(String command) {
        if (command == null) {
            return "";
        }
        String normalized = command.trim();
        if ("autopickup".equals(normalized)) return "toggle_auto_pickup";
        if ("warehouse".equals(normalized)) return "warehouse_manager";
        if ("auto_use_item".equals(normalized)) return "toggle_auto_use_item";
        return normalized;
    }

    private void capturePageState(String command, ModernSettingsTab tab) {
        capturePageState(command, tab, false);
    }

    private void capturePageState(String command, ModernSettingsTab tab, boolean force) {
        if (tab == null || command == null || command.isEmpty()
                || !MainUiLayoutManager.isModernPageStateRecordingEnabled(command)) {
            return;
        }
        long now = System.nanoTime();
        Long previousCapture = pageStateCaptureTimes.get(command);
        if (!force && previousCapture != null
                && now - previousCapture.longValue() < PAGE_STATE_CAPTURE_INTERVAL_NANOS) {
            return;
        }
        pageStateCaptureTimes.put(command, Long.valueOf(now));
        JsonObject snapshot = ModernPageStateRecorder.capture(tab);
        String signature = snapshot.toString();
        if (!signature.equals(pageStateSignatures.get(command))) {
            pageStateSignatures.put(command, signature);
            pendingPageStates.put(command, snapshot);
        }
    }

    private void restorePageState(String command, ModernSettingsTab tab) {
        if (tab == null || command == null || command.isEmpty()
                || !MainUiLayoutManager.isModernPageStateRecordingEnabled(command)) {
            return;
        }
        JsonObject state = MainUiLayoutManager.getModernPageState(command);
        if (state != null) {
            ModernPageStateRecorder.restore(tab, state);
            tab.refreshAfterStateRestore();
        }
    }

    private void captureActiveSettingsState() {
        ModernSettingsTab tab = getActiveSettingsTab();
        if (tab != null) {
            capturePageState(stateCommandForTab(activeTab), tab, true);
        }
    }

    private void captureAllSettingsStates() {
        for (Map.Entry<TabKey, ModernSettingsTab> entry : settingsTabs.entrySet()) {
            capturePageState(stateCommandForTab(entry.getKey()), entry.getValue(), true);
        }
    }

    private void flushPageStates() {
        if (pendingPageStates.isEmpty()) {
            return;
        }
        if (MainUiLayoutManager.isModernStateRecordingEnabled()) {
            for (Map.Entry<String, JsonObject> entry : pendingPageStates.entrySet()) {
                if (MainUiLayoutManager.isModernPageStateRecordingEnabled(entry.getKey())) {
                    MainUiLayoutManager.setModernPageState(entry.getKey(), entry.getValue());
                }
            }
        }
        pendingPageStates.clear();
    }
    ...
    private ModernTabStripLayout.Layout buildTabStripLayout(ModernMainLayout.Rect content) {
        int[] widths = new int[openTabs.size()];
        for (int i = 0; i < openTabs.size(); i++) {
            widths[i] = getMinimumTabWidth(openTabs.get(i));
        }
        ModernTabStripLayout.Orientation orientation = MainUiLayoutManager.isModernTabStripVertical()
                ? ModernTabStripLayout.Orientation.VERTICAL
                : ModernTabStripLayout.Orientation.HORIZONTAL;
        return ModernTabStripLayout.calculate(content, orientation, widths, tabStripInnerWidth,
                currentTabStripMinWidth(), tabStripHeight);
    }
    ...
    private int getMinimumTabWidth(TabKey tab) {
        ...
        int titleWidth = fontRenderer == null ? ModernTabStripLayout.MIN_TITLE_WIDTH
                : fontRenderer.getStringWidth(title);
        return ModernTabStripLayout.tabWidthForTitle(titleWidth, tab.isDashboard());
    }
    ...
    private void ensureActiveTabVisible() {
        ...
        ModernMainLayout.Rect content = lastLayout == null ? null : lastLayout.content;
        ModernTabStripLayout.Layout stripLayout = lastTabStrip;
        if (content != null) {
            stripLayout = buildTabStripLayout(content);
            lastTabStrip = stripLayout;
        }
        if (stripLayout == null) {
            return;
        }
        ...
        tabScrollOffset = clamp(tabScrollOffset, 0, maxTabScrollOffset);
        for (int i = 0; i < openTabs.size(); i++) {
            if (!openTabs.get(i).equals(activeTab)) {
                continue;
            }
            ModernMainLayout.Rect tabBounds = stripLayout.tabAt(i, tabScrollOffset);
            if (stripLayout.vertical()) {
                if (tabBounds.y < stripLayout.viewport.y) {
                    tabScrollOffset = clamp(tabScrollOffset - (stripLayout.viewport.y - tabBounds.y), 0,
                            maxTabScrollOffset);
                } else if (tabBounds.bottom() > stripLayout.viewport.bottom()) {
                    tabScrollOffset = clamp(tabScrollOffset + (tabBounds.bottom() - stripLayout.viewport.bottom()), 0,
                            maxTabScrollOffset);
                }
            } else if (tabBounds.x < stripLayout.viewport.x) {
                tabScrollOffset = clamp(tabScrollOffset - (stripLayout.viewport.x - tabBounds.x), 0,
                        maxTabScrollOffset);
            } else if (tabBounds.right() > stripLayout.viewport.right()) {
                tabScrollOffset = clamp(tabScrollOffset + (tabBounds.right() - stripLayout.viewport.right()), 0,
                        maxTabScrollOffset);
            }
            return;
        }
    ...
    private boolean isCardMouseHint(String line) {
        ...
        return compact.startsWith("gui.modern.main.u110") || compact.startsWith("gui.modern.main.u111")
                || compact.startsWith("gui.modern.main.u112") || compact.startsWith("gui.modern.main.u113")
                || compact.startsWith("leftclick") || compact.startsWith("rightclick")
                || compact.startsWith("leftbutton") || compact.startsWith("rightbutton")
                || compact.startsWith("左键") || compact.startsWith("右键")
                || compact.startsWith("左击") || compact.startsWith("右击");
    }
    ...
    private List<ToolEntry> getToolEntries() {
        ...
        entries.add(new ToolEntry("mcp_settings", "gui.mcp.title"));
        entries.add(new ToolEntry("notes", "gui.modern.utilroute.notes"));
        return entries;
    }

    private List<ToolEntry> getPinnedToolEntries() {
        Set<String> pinnedActions = MainUiLayoutManager.getModernPinnedTools();
        List<ToolEntry> entries = new ArrayList<>();
        if (pinnedActions.isEmpty()) {
            return entries;
        }
        for (ToolEntry entry : getToolEntries()) {
            if (pinnedActions.contains(entry.action)) {
                entries.add(entry);
            }
        }
        return entries;
