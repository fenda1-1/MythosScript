...
public class ModernPathSequencePicker {
    ...
    private final ModernHoverScrollbar categoryScrollbar = new ModernHoverScrollbar();
    private final ModernHoverScrollbar sequenceScrollbar = new ModernHoverScrollbar();
    ...
    public void open(String currentSequence) {
        ...
        categoryScrollbar.endDrag();
        sequenceScrollbar.endDrag();
        groups.clear();
    ...
    private void drawCategories(FontRenderer font, int mouseX, int mouseY) {
        ...
        categoryScroll = clamp(categoryScroll, 0, maxScroll);
        ...
        for (int i = categoryScroll; i < visible.size() && i < categoryScroll + rows; i++) {
            ...
            ModernMainLayout.Rect row = new ModernMainLayout.Rect(categoryBounds.x + 6, y,
                    Math.max(1, ModernHoverScrollbar.contentWidth(clip.width) - 3), 22);
        ...
        categoryScrollbar.draw(clip, categoryScroll, maxScroll, rows, visible.size(), mouseX, mouseY,
                value -> categoryScroll = value);
    }
    ...
    private void drawSequences(FontRenderer font, int mouseX, int mouseY) {
        ...
        int maxScroll = Math.max(0, names.size() - rows);
        sequenceScroll = clamp(sequenceScroll, 0, maxScroll);
        ...
        for (int i = sequenceScroll; i < names.size() && i < sequenceScroll + rows; i++) {
            ...
            ModernMainLayout.Rect row = new ModernMainLayout.Rect(sequenceBounds.x + 6, y,
                    Math.max(1, ModernHoverScrollbar.contentWidth(clip.width) - 3), 24);
        ...
        ModernUiRenderer.endClip();
        sequenceScrollbar.draw(clip, sequenceScroll, maxScroll, rows, names.size(), mouseX, mouseY,
                value -> sequenceScroll = value);
    ...
    public boolean mouseClicked(int mouseX, int mouseY) {
        ...
        if (categoryScrollbar.beginDrag(mouseX, mouseY) || sequenceScrollbar.beginDrag(mouseX, mouseY)) return true;
    ...
    public boolean mouseClickMove(int mouseX, int mouseY, int clickedMouseButton, long timeSinceLastClick) {
        if (!open || clickedMouseButton != 0) return false;
        if (categoryScrollbar.isDragging()) return categoryScrollbar.applyDrag(mouseX, mouseY);
        if (sequenceScrollbar.isDragging()) return sequenceScrollbar.applyDrag(mouseX, mouseY);
        return false;
    }

    public boolean mouseReleased(int mouseX, int mouseY, int state) {
        if (!open) return false;
        if (state == 0 && (categoryScrollbar.isDragging() || sequenceScrollbar.isDragging())) {
            categoryScrollbar.endDrag();
            sequenceScrollbar.endDrag();
            return true;
        }
