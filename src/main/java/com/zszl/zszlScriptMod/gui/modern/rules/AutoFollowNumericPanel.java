...
final class AutoFollowNumericPanel implements ModernFormWidget {
    ...
    AutoFollowNumericPanel(String[] labels, ModernFormSettingsTab.TextValue... values) {
        this.labels=labels; this.values=values; fields=new GuiTextField[values.length];
    }
    ...
    @Override public void draw(FontRenderer font,Rect b,int mx,int my) {
        ...
        for(int i=0;i<values.length;i++) {
            if(fields[i]==null) { fields[i]=new GuiTextField(i,font,0,0,1,18); fields[i].setEnableBackgroundDrawing(false); fields[i].setMaxStringLength(32); }
            GuiTextField f=fields[i]; if(!f.isFocused()) f.setText(values[i].get());
            ...
            f.x=r.x+6; f.y=r.y+7; f.width=r.width-12; f.height=12; f.drawTextBox();
        }
    }
    ...
    @Override public void blur() { for(GuiTextField f:fields) if(f!=null) f.setFocused(false); }
