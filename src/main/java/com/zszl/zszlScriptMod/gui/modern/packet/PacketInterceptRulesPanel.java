...
final class PacketInterceptRulesPanel extends PacketPanelBase {
    ...
    private void drawCategoryNavigation(FontRenderer font, int mx, int my) {
        ...
        ModernUiRenderer.drawSubtlePanel(listBounds.x + 6, y, 48, 20, 3,
                "__all__".equals(selectedCategory) ? 0xFF2C3D49 : ModernUiRenderer.SURFACE,
        ...
        for (String value : state.categories()) {
            ...
            ModernUiRenderer.drawSubtlePanel(r.x, r.y, r.width, r.height, 3,
                    value.equals(selectedCategory) ? 0xFF2C3D49 : r.contains(mx, my) ? ModernUiRenderer.SURFACE_HOVER : ModernUiRenderer.SURFACE,
    ...
    private void configureNavigationActions() {
        ...
        navigationActions.action("move", "gui.modern.nav.move", false, false, () -> selected >= 0, () -> navigationActions.prompt("gui.modern.nav.move", selectedCategory, value -> { sync(); state.rules().get(selected).category = value; selectedCategory = value; select(selected); }));
    ...
    private void openInterceptContextMenu(int x, int y) {
        ...
        if (selected >= 0 && !"__all__".equals(selectedCategory)) {
            items.add(new PacketContextMenu.Item("移动到当前分组", () -> state.rules().get(selected).category = selectedCategory));
        }
