...
public final class ModernMainLayout {
    ...
    private static final int COLLAPSED_HEADER_HEIGHT = 14;
    private static final int HEADER_TOGGLE_WIDTH = 48;
    private static final int HEADER_TOGGLE_HEIGHT = 14;
    ...
    public static Layout calculate(int screenWidth, int screenHeight, Rect requestedShell, boolean sidebarCollapsed,
        ...
        return calculate(screenWidth, screenHeight, requestedShell, sidebarCollapsed, requestedSidebarWidth, false);
    }

    public static Layout calculate(int screenWidth, int screenHeight, Rect requestedShell, boolean sidebarCollapsed,
            int requestedSidebarWidth, boolean headerCollapsed) {
        int safeScreenWidth = Math.max(1, screenWidth);
        int safeScreenHeight = Math.max(1, screenHeight);
        ...
        int headerHeight = headerCollapsed
                ? Math.min(COLLAPSED_HEADER_HEIGHT, Math.max(1, shellHeight - 1))
                : Math.max(MIN_HEADER_HEIGHT, Math.min(MAX_HEADER_HEIGHT, shellHeight / 6));
        ...
        Rect headerToggle = new Rect(shell.x + Math.max(0, (shell.width - HEADER_TOGGLE_WIDTH) / 2),
                headerCollapsed ? shell.y : header.bottom() - HEADER_TOGGLE_HEIGHT / 2,
                Math.min(HEADER_TOGGLE_WIDTH, shell.width), HEADER_TOGGLE_HEIGHT);
        return new Layout(shell, header, sidebar, content, effectiveSidebarCollapsed, headerCollapsed, headerToggle);
    }
    ...
    public static final class Layout {
        ...
        public final boolean headerCollapsed;
        public final Rect headerToggle;

        private Layout(Rect shell, Rect header, Rect sidebar, Rect content, boolean sidebarCollapsed,
                boolean headerCollapsed, Rect headerToggle) {
            this.shell = shell;
            this.header = header;
    ...
    public static Grid createDashboardGrid(Rect content, int reservedTop, int reservedBottom, int gap,
        ...
        return createDashboardGrid(content, reservedTop, reservedBottom, gap, requestedColumns, requestedCardHeight, 0);
    }

    public static Grid createDashboardGrid(Rect content, int reservedTop, int reservedBottom, int gap,
            int requestedColumns, int requestedCardHeight, int requestedCardWidth) {
        Rect safeContent = content == null ? new Rect(0, 0, 1, 1) : content;
        int safeGap = Math.max(4, gap);
        ...
        int cardHeight = Math.max(34, Math.min(160, requestedCardHeight));
        int desiredCardWidth = requestedCardWidth > 0
                ? Math.max(88, Math.min(420, requestedCardWidth))
                : cardHeight <= 40 ? 138 : cardHeight >= 58 ? 204 : 168;
        int maxResponsiveColumns = Math.max(1, (gridWidth + safeGap) / (desiredCardWidth + safeGap));
        int columns;
        ...
        int cardWidth = requestedCardWidth > 0
                ? Math.max(1, Math.min(gridWidth, requestedCardWidth))
                : Math.max(1, (gridWidth - safeGap * Math.max(0, columns - 1)) / columns);
        return new Grid(new Rect(gridX, gridY, gridWidth, gridHeight), columns, cardWidth, cardHeight, safeGap);
    }
