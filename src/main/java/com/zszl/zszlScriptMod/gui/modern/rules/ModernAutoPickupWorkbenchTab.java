...
public final class ModernAutoPickupWorkbenchTab implements ModernSettingsTab {
    ...
    private static final String[] EDITOR_SECTIONS = {
            "gui.modern.pickup_wb.u001", "gui.modern.pickup_wb.u002", "gui.modern.pickup_wb.u003", "gui.modern.pickup_wb.u004", "gui.modern.pickup_wb.u005", "gui.modern.pickup_wb.u006"
    };
    ...
    public void draw(FontRenderer fontRenderer, ModernMainLayout.Rect requested, int mouseX, int mouseY) {
        ...
        int sectionWidth = editorBounds.width >= 220
                ? Math.min(SECTION_NAV_WIDTH, Math.max(78, editorBounds.width - 145)) : 0;
        if (sectionWidth > 0) {
            sectionNavBounds = new ModernMainLayout.Rect(editorBounds.x + 7, editorBounds.y + 7, sectionWidth,
                    Math.max(1, editorBounds.height - 14));
    ...
    private void configureNavigationActions() {
        navigationActions.action("move", "gui.modern.nav.move", false, false, () -> state.selected() != null, () -> { if (prepareDraft() && state.selected() != null) navigationActions.prompt("gui.modern.nav.move", state.selected().category, value -> { state.selected().category = value; state.selectCategory(value); categoryField.setText(value); rebuildEditor(); }); });
    ...
    private void drawSectionNavigation(FontRenderer font, int mouseX, int mouseY) {
        ...
        ModernUiRenderer.drawText(font, "gui.modern.pickup_wb.u024", sectionNavBounds.x + 9, sectionNavBounds.y + 10,
        ...
        for (int i = 0; i < EDITOR_SECTIONS.length; i++) {
            ...
            ModernUiRenderer.drawText(font, EDITOR_SECTIONS[i], row.x + 8,
                    ...
                    selected ? ModernUiRenderer.TEXT : ModernUiRenderer.SUBTLE_TEXT, Math.max(1, row.width - 16));
            if (intersectsVertically(row, clip)) {
    ...
    private ModernSettingsTab buildEditor(final AutoPickupRule rule) {
        ModernFormSettingsTab.Builder<AutoPickupRule> builder = ModernFormSettingsTab.builder(
                "gui.modern.pickup_wb.u040", "gui.modern.pickup_wb.u041", "gui.modern.pickup_wb.u042",
                ...
                .text("gui.modern.pickup_wb.u047", "gui.modern.pickup_wb.u048",
                        text(() -> safe(rule.category), value -> rule.category = value), "gui.modern.pickup_wb.u049", 80)
                .toggle("gui.modern.pickup_wb.u050", "gui.modern.pickup_wb.u051",
                ...
                .integer("gui.modern.pickup_wb.u062", "gui.modern.pickup_wb.u063",
                        ...
                        Integer.MAX_VALUE)
                .action("gui.modern.pickup_wb.u064", "gui.modern.pickup_wb.u065", "gui.modern.pickup_wb.u066",
                        ModernFormSettingsTab.ActionStyle.SECONDARY, tab -> fillPlayerCoordinates())
                ...
                .readOnly("gui.modern.pickup_wb.u077", "gui.modern.pickup_wb.u078",
                        () -> String.valueOf(safeItemEntries(rule.itemWhitelistEntries, rule.itemWhitelist).size()))
                .action("gui.modern.pickup_wb.u079", "gui.modern.pickup_wb.u080",
                        entryButtonLabel("gui.modern.pickup_wb.u081", rule.itemWhitelistEntries, rule.itemWhitelist),
                        ModernFormSettingsTab.ActionStyle.SECONDARY, tab -> openFilterList(true))
                ...
                .readOnly("gui.modern.pickup_wb.u085", "gui.modern.pickup_wb.u078",
                        () -> String.valueOf(safeItemEntries(rule.itemBlacklistEntries, rule.itemBlacklist).size()))
                .action("gui.modern.pickup_wb.u086", "gui.modern.pickup_wb.u087",
                        entryButtonLabel("gui.modern.pickup_wb.u088", rule.itemBlacklistEntries, rule.itemBlacklist),
                        ModernFormSettingsTab.ActionStyle.SECONDARY, tab -> openFilterList(false))
                .section("gui.modern.pickup_wb.u005", "gui.modern.pickup_wb.u089")
                .readOnly("gui.modern.pickup_wb.u090", "gui.modern.pickup_wb.u091",
                        () -> String.valueOf(safeActionEntries(rule.pickupActionEntries).size()))
                .action("gui.modern.pickup_wb.u092", "gui.modern.pickup_wb.u093",
                        "编辑拾取动作 (" + safeActionEntries(rule.pickupActionEntries).size() + ")",
                        ModernFormSettingsTab.ActionStyle.SECONDARY, tab -> openActionList())
    ...
    private void fillPlayerCoordinates() {
        ...
        editorRule.centerX = minecraft.player.posX;
        editorRule.centerY = minecraft.player.posY;
        editorRule.centerZ = minecraft.player.posZ;
        if (editor instanceof ModernFormSettingsTab) {
            ((ModernFormSettingsTab<?>) editor).refreshValues();
        }
        status = "gui.modern.pickup_wb.u111";
    }
    ...
    private boolean handleEntryListClick(int mouseX, int mouseY) {
        ...
        if (contains(overlayDeleteBounds, mouseX, mouseY)) {
            ...
            if (overlay == Overlay.ACTION_LIST) {
                ...
                entryList(overlayWhitelist).remove(overlaySelectedEntryIndex);
            }
        ...
        if (keyword.isEmpty() && overlayNbtTags.isEmpty()) {
            if (overlayEntryIndex >= 0) {
                ...
                if (overlayEntryIndex < entries.size()) {
                    entries.remove(overlayEntryIndex);
    ...
    private String inventoryButtonLabel(AutoPickupRule rule) {
        ...
        return count == 0 ? "gui.modern.pickup_wb.u163" : "gui.modern.pickup_wb.u164" + count + " 格）";
    }
    ...
    private double parseDouble(String value, double fallback, String label) {
        try {
            ...
            if (editorInputError.isEmpty()) {
                editorInputError = label + "gui.modern.pickup_wb.u177";
            }
            return fallback;
        }
    }
