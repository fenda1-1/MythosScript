...
final class PacketViewerPanel extends PacketPanelBase {
    ...
    PacketViewerPanel(PacketWorkbenchTab owner, List<PacketCaptureHandler.CapturedPacketData> packets, String title, boolean readOnly, String initialDirection) {
        super(owner, "gui.modern.pktview.u004");
    ...
    private void drawTimeline(FontRenderer renderer, int mouseX, int mouseY) {
        ...
        for (int i = timelineScroll; i < Math.min(events.size(), timelineScroll + visibleRows); i++) {
            ...
            ModernUiRenderer.drawSubtlePanel(listClip.x, rowY, ModernHoverScrollbar.contentWidth(listClip.width), 20, 3,
                    i == selectedTimeline ? 0xFF2C3D49 : hover ? ModernUiRenderer.SURFACE_HOVER : ModernUiRenderer.SHELL_RAISED,
    ...
    private void drawPackets(FontRenderer renderer, int mouseX, int mouseY) {
        ...
        for (int row = packetScroll; row < Math.min(packetRowCount(), packetScroll + rows); row++) {
            for (int col = 0; col < columns; col++) {
                ...
                ModernUiRenderer.drawSubtlePanel(cardX, rowY, cardWidth, cardHeight, 4,
                    chosen ? 0xFF2C3D49 : hover ? ModernUiRenderer.SURFACE_HOVER : ModernUiRenderer.SHELL_RAISED,
        ...
        if (visible.isEmpty()) text(renderer, "gui.modern.pktview.u017", rightBounds.x + 12, rightBounds.y + 48, ModernUiRenderer.MUTED_TEXT, rightBounds.width - 24);
        int packetMax = packetScrollMax();
        ModernMainLayout.Rect packetClip = new ModernMainLayout.Rect(rightBounds.x + 4, rightBounds.y + 28,
                rightBounds.width - 8, Math.max(1, rightBounds.height - 32));
        if (packetMax > 0) {
    ...
    private void drawFooter(FontRenderer renderer, int mouseX, int mouseY) {
        String[] labels = { readOnly ? "gui.modern.pktview.u018" : "gui.modern.pktview.u019", "gui.modern.pktview.u020", "gui.modern.pktview.u021", "gui.modern.pktview.u022", "gui.modern.pktview.u023", "gui.modern.pktview.u024", "gui.modern.pktview.u025", "gui.modern.pktview.u026", "gui.modern.pktview.u027" };
        footerActions.clear();
        ...
        for (int i = 0; i < labels.length; i++) {
            ...
            boolean enabled;
            switch (i) {
                case 0: enabled = readOnly || !selected.isEmpty(); break;
                case 1: enabled = !readOnly; break;
                case 2: case 3: case 4: case 7: enabled = !selected.isEmpty() && (!readOnly || (i != 3)); break;
    ...
    private int packetVisibleRows() {
        return rightBounds == null ? 1 : Math.max(1, (rightBounds.height - 34) / Math.max(1, packetCardHeight()));
    }
    ...
    private int packetHitOffsetY(int y) {
        ...
        int rowStep = Math.max(1, packetRowHeight > 0 ? packetRowHeight : packetCardHeight());
        int offset = (y - (rightBounds.y + 28)) % rowStep;
    ...
    private void saveSnapshot() {
        ...
        owner.panels().push(new PacketModalPanel(owner, "gui.modern.pktview.u023", "gui.modern.pktview.u035", "gui.modern.pktview.u036" + new SimpleDateFormat("yyyyMMdd-HHmmss").format(new Date()), true,
                value -> {
