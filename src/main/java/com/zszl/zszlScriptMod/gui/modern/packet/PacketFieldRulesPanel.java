...
final class PacketFieldRulesPanel extends PacketPanelBase {
    ...
    private ModernMainLayout.Rect categoryBounds, ruleListBounds, categorySplitBounds;
    ...
    private void configureNavigationActions() {
        ...
        navigationActions.action("move", "gui.modern.nav.move", false, false, () -> selected >= 0, () -> navigationActions.prompt("gui.modern.nav.move", selectedCategory, value -> { sync(); state.models().get(selected).category = value; selectedCategory = value; select(selected); }));
    ...
    private void drawCategoryTree(FontRenderer font, int mx, int my) {
        ...
        ModernUiRenderer.drawSubtlePanel(all.x, all.y, all.width, all.height, 3,
                "__all__".equals(selectedCategory) ? 0xFF2C3D49 : all.contains(mx, my) ? ModernUiRenderer.SURFACE_HOVER : ModernUiRenderer.SHELL_RAISED,
        ...
        for (int i = 0; i < categories.size() && y + 20 <= categoryBounds.bottom(); i++, y += 24) {
            ...
            ModernUiRenderer.drawSubtlePanel(row.x, row.y, row.width, row.height, 3,
                    selected ? 0xFF2C3D49 : row.contains(mx, my) ? ModernUiRenderer.SURFACE_HOVER : ModernUiRenderer.SHELL_RAISED,
    ...
    private void drawEditor(FontRenderer font, int mx, int my) {
        ...
        y = fieldRow(font, "gui.modern.pktfield.u011", name, editorBounds.x + pad, y, fieldWidth, safeModel().name); y = fieldRow(font, "gui.modern.pktfield.u012", channel, editorBounds.x + pad, y, fieldWidth, safeModel().channel); y = fieldRow(font, "分类", category, editorBounds.x + pad, y, fieldWidth, safeModel().category);
    ...
    @Override protected boolean handleBodyClick(int x, int y, int button) {
        ...
        if (fieldClick(search, x, y, button) || fieldClick(name, x, y, button) || fieldClick(channel, x, y, button) || fieldClick(category, x, y, button) || fieldClick(pattern, x, y, button) || fieldClick(group, x, y, button) || fieldClick(variable, x, y, button) || fieldClick(defaultValue, x, y, button) || fieldClick(note, x, y, button)) return true;
    ...
    private void openFieldContextMenu(int x, int y) {
        ...
        if (selected >= 0 && !"__all__".equals(selectedCategory)) {
            items.add(new PacketContextMenu.Item("移动到当前分组", () -> { sync(); state.models().get(selected).category = selectedCategory; }));
        }
