...
public final class ModernConditionalExecutionWorkbenchTab implements ModernSettingsTab {
    ...
    private void configureNavigationActions() {
        ...
        navigationActions.action("category_delete", "gui.modern.nav.category_delete", false, true,
                () -> !isDefaultCategory(state.selectedCategory()), this::navigationCategoryDelete);
    ...
    private boolean navigationCategoryDelete() {
            ...
            navigationActions.confirm("gui.modern.nav.category_delete", () -> {
                if (!syncEditorDraft() || !state.deleteCategory(category)) {
    ...
    private ModernSettingsTab buildEditor(final ConditionalRule rule) {
        ModernFormSettingsTab.Builder<ConditionalRule> builder = ModernFormSettingsTab
                ...
                .footerVisible(false)
                .headerToggle(bool(() -> rule.enabled, value -> rule.enabled = value), "gui.modern.conditional.u055")
                ...
                .choice("gui.modern.conditional.u052", "gui.modern.conditional.u053",
                ...
                .action("坐标操作", "灵魂出窍：左键选中心，右键确定半径，中键确认，Esc 取消。", "开始可视化取点",
                        ModernFormSettingsTab.ActionStyle.PRIMARY, tab -> fillPlayerCoordinates(rule))
        ...
        builder.custom(new RuleCardsPanel(() -> sequenceActionDescriptions(rule), null, null, null, null));
    ...
    private void fillPlayerCoordinates(ConditionalRule rule) {
        ...
        final ConditionalRule selected = rule;
        AutoFollowAreaPicker.startRadius((center, radius) -> {
            selected.centerX = center.getX() + 0.5D;
            selected.centerY = center.getY();
            selected.centerZ = center.getZ() + 0.5D;
            selected.range = radius;
    ...
    private static String normalizeRuleName(String name) {
        ...
        if ("gui.modern.conditional.u157".equalsIgnoreCase(normalized)) {
            return ModernFormI18n.tr("gui.modern.conditional.u157");
        }
        if ("gui.modern.conditional.u158".equalsIgnoreCase(normalized)) {
            return ModernFormI18n.tr("gui.modern.conditional.u158");
        }
        return normalized;
    }
    ...
    private static final class ConditionalWorkbenchState {
        ...
        private ConditionalWorkbenchState(List<ConditionalRule> source, List<String> categories,
            ...
            if (source != null) {
                for (ConditionalRule sourceRule : source) {
                    ...
                    draft.name = normalizeRuleName(draft.name);
                    draft.normalize();
                    rules.add(draft);
        ...
        private void addRule(String category) {
            ...
            rule.name = uniqueName(ModernFormI18n.tr("gui.modern.conditional.u157"));
            rules.add(rule);
            selected = rule;
        ...
        private String uniqueName(String base) {
            ...
            if (normalizedBase.isEmpty()) {
                normalizedBase = ModernFormI18n.tr("gui.modern.conditional.u158");
            }
        ...
        private void normalizeDrafts() {
            for (ConditionalRule rule : rules) {
                if (rule != null) {
                    rule.category = normalizeCategory(rule.category);
                    rule.name = normalizeRuleName(rule.name);
                    rule.normalize();
                }
            }
        }
