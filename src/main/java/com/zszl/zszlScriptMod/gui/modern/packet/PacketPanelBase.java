...
abstract class PacketPanelBase extends ModernEmbeddedPanel {
    ...
    PacketPanelBase(PacketWorkbenchTab owner, String title) { this(owner, title, true, true); }
    PacketPanelBase(PacketWorkbenchTab owner, String title, boolean backButton) {
        this(owner, title, backButton, true);
    }
    PacketPanelBase(PacketWorkbenchTab owner, String title, boolean backButton, boolean header) {
        super(title); this.owner = owner; this.backButton = backButton; this.header = header;
    }
    ...
    @Override public final boolean mouseClicked(int mouseX, int mouseY, int mouseButton) {
        ...
        if (header && backButton && mouseButton == 0 && backBounds().contains(mouseX, mouseY)) { owner.requestBack(); return true; }
        if (header && mouseButton == 0 && moreBounds != null && moreBounds.contains(mouseX, mouseY)) {
    ...
    protected void centeredLabel(FontRenderer renderer, String value, ModernMainLayout.Rect bounds, int color) {
        ...
        GlStateManager.scale(scale, scale, 1.0F);
        renderer.drawString(text, -width / 2.0F, -renderer.FONT_HEIGHT / 2.0F,
