...
public final class ModernBaritoneSettingsTab implements ModernSettingsTab {
    ...
    private static final int PAGE_SIZE_BASE = 16;
    ...
    private void recalculatePaging() {
        int pageSize = Math.max(1, columns * PAGE_SIZE_BASE / 2);
    ...
    public void draw(FontRenderer fontRenderer, ModernMainLayout.Rect requestedBounds, int mouseX, int mouseY) {
        ...
        int listY = toolbarY + (compact ? 58 : 31);
        contentClipBounds = new ModernMainLayout.Rect(panelBounds.x + 8, listY,
                Math.max(1, panelBounds.width - 16), Math.max(1, footerY - listY - 7));
        columns = chooseColumns(contentClipBounds.width);
        recalculatePaging();
    ...
    private void drawToolbar(int y, int mouseX, int mouseY) {
        ...
        if (panelBounds.width < 470) {
            ...
            reloadBounds = new ModernMainLayout.Rect(x, y + 28, width, 23);
            drawButton(typeFilterBounds, ModernFormI18n.tr("gui.modern.baritone_set.fmt.type", typeLabel()), false, typeFilterBounds.contains(mouseX, mouseY));
            drawButton(modifiedFilterBounds, modifiedOnly ? "gui.modern.baritone_set.u008" : "gui.modern.baritone_set.u009", modifiedOnly,
                    modifiedFilterBounds.contains(mouseX, mouseY));
        ...
        ModernUiRenderer.drawText(fontRenderer, status, panelBounds.right() - 190, y + 7,
                ModernUiRenderer.MUTED_TEXT, 178);
    }
    ...
    public boolean mouseClicked(int mouseX, int mouseY, int mouseButton) {
        ...
        for (SettingDef def : pageSettings()) {
            ...
            if (!contains(control, mouseX, mouseY)) {
                continue;
            }
            if (isBlockList(def)) {
    ...
    private void cycleTypeFilter() {
        ...
        typeFilter = TYPE_FILTERS[(index + 1) % TYPE_FILTERS.length];
        page = 0;
        refreshFilteredSettings();
    }
    ...
    private int chooseColumns(int width) {
        if (width >= 1050) {
            return 4;
        }
        if (width >= 760) {
    ...
    private int pageSize() {
        return Math.max(1, columns * PAGE_SIZE_BASE / 2);
    }
