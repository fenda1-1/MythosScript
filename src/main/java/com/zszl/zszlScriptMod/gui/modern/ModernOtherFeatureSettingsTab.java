...
public final class ModernOtherFeatureSettingsTab {
    ...
    private static void appendSpecialFields(ModernFormSettingsTab.Builder<Object> builder, final FeatureAccess access) {
        ...
        } else if (access.kind == Kind.RENDER && "entity_tags".equals(access.id)) {
            builder.section(text("gui.other_feature.entity_tag_filters", "实体筛选"),
                    text("gui.other_feature.entity_tag_filters_tip", "选择哪些实体显示头顶标签；总开关在页面标题右侧。"))
                    .toggle(text("gui.other_feature.entity_tag_players", "玩家"),
                            text("gui.other_feature.entity_tag_players_tip", "显示玩家的实体标签。"),
                            access.entityTagFilterValue(0))
                    .toggle(text("gui.other_feature.entity_tag_monsters", "怪物"),
                            text("gui.other_feature.entity_tag_monsters_tip", "显示怪物的实体标签。"),
                            access.entityTagFilterValue(1))
                    .toggle(text("gui.other_feature.entity_tag_animals", "动物与 NPC"),
                            text("gui.other_feature.entity_tag_animals_tip", "显示动物、村民和其他 NPC 的实体标签。"),
                            access.entityTagFilterValue(2));
            builder.section(text("gui.other_feature.entity_tag_content", "显示内容"),
                    text("gui.other_feature.entity_tag_content_tip", "只保留需要查看的信息，减少画面遮挡。"))
                    .toggle(text("gui.other_feature.entity_tag_health", "生命值"),
                            text("gui.other_feature.entity_tag_health_tip", "在名称下显示实体当前生命值。"),
                            access.entityTagContentValue(0))
                    .toggle(text("gui.other_feature.entity_tag_distance", "距离"),
                            text("gui.other_feature.entity_tag_distance_tip", "在名称下显示实体与玩家的距离。"),
                            access.entityTagContentValue(1))
                    .toggle(text("gui.other_feature.entity_tag_held_item", "手持物"),
                            text("gui.other_feature.entity_tag_held_item_tip", "实体手持物不为空时显示手持物名称。"),
                            access.entityTagContentValue(2));
            builder.section(text("gui.other_feature.entity_display_range", "实体显示范围"),
                    text("gui.other_feature.entity_display_range_tip", "实体标签默认在玩家十格内显示；可按需扩大到 100 格。"))
                    .integer(text("gui.other_feature.entity_display_distance", "最大显示距离"),
                            text("gui.other_feature.entity_display_distance_tip", "范围 1–100 格，默认 10 格。"), access.entityDistanceValue(),
                            RenderFeatureManager.ENTITY_DISTANCE_MIN, RenderFeatureManager.ENTITY_DISTANCE_MAX);
        } else if (access.kind == Kind.RENDER && "entity_info".equals(access.id)) {
            builder.section(text("gui.other_feature.entity_display_range", "实体显示范围"),
                    text("gui.other_feature.entity_display_range_tip", "设置实体信息面板的最大显示距离，范围 1–100 格。"))
                    .integer("最大显示距离", "范围 1–100 格。", access.entityDistanceValue(),
                            RenderFeatureManager.ENTITY_DISTANCE_MIN, RenderFeatureManager.ENTITY_DISTANCE_MAX);
        }
    }
    ...
    private static final class FeatureAccess {
        ...
        private ModernFormSettingsTab.IntValue entityDistanceValue() {
            return new ModernFormSettingsTab.IntValue() {
                @Override public int get() {
                    return "entity_info".equals(id)
                            ? RenderFeatureManager.getEntityInfoMaxDistance()
                            : RenderFeatureManager.getEntityTagMaxDistance();
                }

                @Override public void set(int value) {
                    if ("entity_info".equals(id)) {
                        RenderFeatureManager.setEntityInfoMaxDistance(value);
                    } else {
                        RenderFeatureManager.setEntityTagMaxDistance(value);
                    }
                }
            };
        }

        private ModernFormSettingsTab.BooleanValue entityTagFilterValue(final int which) {
            return new ModernFormSettingsTab.BooleanValue() {
                @Override public boolean get() {
                    return which == 0 ? RenderFeatureManager.entityTagPlayers
                            : which == 1 ? RenderFeatureManager.entityTagMonsters
                            : RenderFeatureManager.entityTagAnimals;
                }

                @Override public void set(boolean value) {
                    if (which == 0) RenderFeatureManager.entityTagPlayers = value;
                    else if (which == 1) RenderFeatureManager.entityTagMonsters = value;
                    else RenderFeatureManager.entityTagAnimals = value;
                }
            };
        }

        private ModernFormSettingsTab.BooleanValue entityTagContentValue(final int which) {
            return new ModernFormSettingsTab.BooleanValue() {
                @Override public boolean get() {
                    return which == 0 ? RenderFeatureManager.entityTagShowHealth
                            : which == 1 ? RenderFeatureManager.entityTagShowDistance
                            : RenderFeatureManager.entityTagShowHeldItem;
                }

                @Override public void set(boolean value) {
                    if (which == 0) RenderFeatureManager.entityTagShowHealth = value;
                    else if (which == 1) RenderFeatureManager.entityTagShowDistance = value;
                    else RenderFeatureManager.entityTagShowHeldItem = value;
                }
            };
        }
