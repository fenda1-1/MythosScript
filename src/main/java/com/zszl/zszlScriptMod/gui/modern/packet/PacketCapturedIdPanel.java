...
final class PacketCapturedIdPanel extends PacketPanelBase {
    ...
    private ModernMainLayout.Rect categorySplitBounds;
    ...
    private void configureNavigationActions() {
        ...
        navigationActions.action("move", "gui.modern.nav.move", false, false, () -> selected >= 0, () -> navigationActions.prompt("gui.modern.nav.move", category.text(), value -> category.setText(value)));
    ...
    private void drawEditor(FontRenderer font, int mx, int my) {
        ...
        y = row(font, "gui.modern.pktid.u019", name, y, fw); y = row(font, "gui.modern.pktid.u020", display, y, fw); y = row(font, "gui.modern.pktid.u021", note, y, fw); y = row(font, "gui.modern.pktid.u022", aliases, y, fw); y = row(font, "gui.modern.pktid.u012", category, y, fw); y = row(font, "gui.modern.pktid.u023", channel, y, fw); y = row(font, "Pattern", pattern, y, fw); y = row(font, "Offset", offset, y, fw); y = row(font, "Group", group, y, fw); y = row(font, "gui.modern.pktid.u024", bytes, y, fw);
        int sequenceY = y;
    ...
    private void drawFlag(FontRenderer font, ModernMainLayout.Rect r, String value, int mx, int my, boolean selected) { boolean hover = r.contains(mx, my); ModernUiRenderer.drawSubtlePanel(r.x, r.y, r.width, r.height, 3, selected ? 0xFF2C3D49 : hover ? ModernUiRenderer.SURFACE_HOVER : ModernUiRenderer.SHELL_RAISED, selected ? ModernUiRenderer.ACCENT : ModernUiRenderer.BORDER_SUBTLE); text(font, value, r.x + 8, r.y + 6, ModernUiRenderer.TEXT, r.width - 16); }
    ...
    @Override protected boolean handleBodyClick(int x, int y, int button) {
        ...
        ModernMainLayout.Rect viewport = editorContentBounds();
        if (viewport.contains(x, y) && (fieldClick(name, x, y, button) || fieldClick(display, x, y, button) || fieldClick(note, x, y, button) || fieldClick(aliases, x, y, button) || fieldClick(category, x, y, button) || fieldClick(channel, x, y, button) || fieldClick(pattern, x, y, button) || fieldClick(offset, x, y, button) || fieldClick(group, x, y, button) || fieldClick(bytes, x, y, button) || fieldClick(sequence, x, y, button) || fieldClick(cooldown, x, y, button))) return true;
    ...
    private CapturedIdRuleManager.RuleEditModel safeModel() { return selected >= 0 && selected < cards.size() ? cards.get(selected).model : new CapturedIdRuleManager.RuleEditModel(); }
     @Override public boolean keyTyped(char c, int code) {
        if (navigationActions.keyTyped(c, code)) return true; if (code == org.lwjgl.input.Keyboard.KEY_DELETE && !ALL.equals(selectedCategory) && !UNGROUPED.equals(selectedCategory)) { if (CapturedIdRuleManager.deleteCategory(selectedCategory)) { selectedCategory = ALL; refresh(); } return true; } return fieldKey(search, c, code) || fieldKey(name, c, code) || fieldKey(display, c, code) || fieldKey(note, c, code) || fieldKey(aliases, c, code) || fieldKey(category, c, code) || fieldKey(channel, c, code) || fieldKey(pattern, c, code) || fieldKey(offset, c, code) || fieldKey(group, c, code) || fieldKey(bytes, c, code) || fieldKey(sequence, c, code) || fieldKey(cooldown, c, code); }
    ...
    private void openCapturedIdContext(int x, int y) {
        ...
        if (selected >= 0) {
            ...
            if (category != null && !ALL.equals(category) && selected < cards.size()) {
                ...
                items.add(new PacketContextMenu.Item("移动到当前分组", () -> CapturedIdRuleManager.moveRuleToCategory(cards.get(selected).index, targetCategory)));
            }
        }
