...
public final class ModernBlockReplacementWorkbenchTab implements ModernSettingsTab {
    ...
    public void draw(FontRenderer fontRenderer, ModernMainLayout.Rect requested, int mouseX, int mouseY) {
        ...
        drawEntries(fontRenderer, mouseX, mouseY);
        drawFooter(fontRenderer, mouseX, mouseY);
        navigationActions.drawOverlay(mouseX, mouseY);
    }

    private void layoutEditorColumns() {
        int gap = editorBounds.height >= 16 ? 6 : 0;
        int available = Math.max(1, editorBounds.height - gap);
        int entryHeight = Math.min(112, Math.max(68, available / 2));
        if (entryHeight >= available) {
            entryHeight = Math.max(1, available - 1);
        }
        int configurationHeight = Math.max(1, available - entryHeight);
        configurationBounds = new ModernMainLayout.Rect(editorBounds.x, editorBounds.y, editorBounds.width,
                configurationHeight);
        entriesBounds = new ModernMainLayout.Rect(editorBounds.x, configurationBounds.bottom() + gap,
                editorBounds.width, entryHeight);
    }
    ...
    private void configureNavigationActions() {
        navigationActions.action("move", "gui.modern.nav.move", false, false, () -> state.selected() != null,
                () -> { if (syncRuleDraft() && state.selected() != null) { syncEntryDraft();
                    navigationActions.prompt("gui.modern.nav.move", state.selected().category, value -> {
    ...
    private void drawRuleRow(FontRenderer fontRenderer, ModernMainLayout.Rect row,
        ...
        String details = (rule.hasValidRegion() ? "gui.modern.replace_wb.u007" : "gui.modern.replace_wb.u008") + " · "
            ...
            for (int i = 0; i < visible; i++) {
                ...
                ModernUiRenderer.drawSubtlePanel(row.x, row.y, row.width, row.height, 4,
                        selected ? ModernUiRenderer.SURFACE_PRESSED : hovered ? ModernUiRenderer.SURFACE_HOVER
                                : ModernUiRenderer.SURFACE,
                        selected ? ModernUiRenderer.ACCENT : ModernUiRenderer.BORDER_SUBTLE);
                ModernUiRenderer.drawStatusDot(row.x + 6, row.y + 7,
                        entry != null && entry.enabled ? ModernUiRenderer.SUCCESS : ModernUiRenderer.MUTED_TEXT);
    ...
    public boolean handleMouseWheel(int wheel, int mouseX, int mouseY) {
        ...
        if (entryListBounds != null && entryListBounds.contains(mouseX, mouseY)) {
            ...
            entryScroll = clamp(entryScroll + (wheel > 0 ? -ENTRY_ROW_HEIGHT : ENTRY_ROW_HEIGHT), 0, entryMaxScroll);
    ...
    public boolean handleEscape() {
        ...
        if (sourceField != null && sourceField.isFocused() || targetField != null && targetField.isFocused()) {
            clearCustomFocus();
            return true;
        }
    ...
    public boolean isTextInputFocused() {
        ...
        return searchField != null && searchField.isFocused()
                ...
                || sourceField != null && sourceField.isFocused()
    ...
    public void save() {
        ...
        try {
            ...
            status = "gui.modern.replace_wb.u036" + safeMessage(exception);
        }
    }
        ...
        for (int ruleIndex = 0; ruleIndex < state.rules().size(); ruleIndex++) {
            ...
            for (int entryIndex = 0; entryIndex < rule.replacements.size(); entryIndex++) {
                ...
                if (entry == null || isBlank(entry.sourceBlockId) || isBlank(entry.targetBlockId)) {
                    return "gui.modern.replace_wb.u037" + safe(rule.name) + "”的第 " + (entryIndex + 1)
                            + " 条目需要填写源方块和目标方块";
                }
            }
        }
    ...
    private void reloadFromSource() {
        ...
        try {
            ...
            pendingReload = false;
            status = "gui.modern.replace_wb.u040" + safeMessage(exception);
        }
    }
    ...
    private ModernFormSettingsTab<BlockReplacementRule> buildEditor(final BlockReplacementRule rule) {
        return ModernFormSettingsTab.builder("gui.modern.replace_wb.u043", "gui.modern.replace_wb.u048",
                ...
                .text("gui.modern.replace_wb.u054", "gui.modern.replace_wb.u055",
                        text(() -> rule.category, value -> rule.category = value), CATEGORY_DEFAULT, 120)
                ...
                .toggle("gui.modern.replace_wb.u061", "gui.modern.replace_wb.u062",
    ...
    private void addRule() {
        ...
        clearPendingDeletes();
        rebuildEditor();
        status = "gui.modern.replace_wb.u089" + safe(rule.name);
    }
    ...
    private void duplicateRule() {
        ...
        clearPendingDeletes();
        rebuildEditor();
        status = "gui.modern.replace_wb.u092" + safe(copy.name);
    }
    ...
    private void addCategory() {
        ...
        if (state.addCategory(name)) {
            ...
            status = "gui.modern.replace_wb.u100" + name;
        } else {
    ...
    private static String safeMessage(RuntimeException exception) {
        return exception == null || isBlank(exception.getMessage()) ? "gui.modern.replace_wb.u128"
