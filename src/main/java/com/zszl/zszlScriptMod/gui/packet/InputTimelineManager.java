...
public final class InputTimelineManager {
    ...
    public static final class InputEventRecord {
        ...
        private InputEventRecord(long eventId, long timestamp, String category, String action, String detail,
                int keyCode, int mouseButton, int sessionId, String guiTitle, String screenSimpleName) {
    ...
    public static synchronized void recordKeyPress(int keyCode) {
        ...
        addEvent(new InputEventRecord(++nextEventId, System.currentTimeMillis(), "键盘", "按键按下", keyName,
                keyCode, -1, session.sessionId, session.guiTitle, session.screenSimpleName));
    }
    ...
    public static synchronized void recordMouseClick(int button) {
        ...
        SessionInfo session = resolveCurrentSession();
        addEvent(new InputEventRecord(++nextEventId, System.currentTimeMillis(), "鼠标", "点击", detail,
                -1, button, session.sessionId, session.guiTitle, session.screenSimpleName));
    }
