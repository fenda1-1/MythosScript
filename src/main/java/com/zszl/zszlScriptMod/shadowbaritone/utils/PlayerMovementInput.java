...
public class PlayerMovementInput extends MovementInput {
    ...
    private boolean shouldBypassHumanLikeMovement() {
        return ParkourRuntimeHelper.getActiveParkourMovement(handler.baritone) != null
                || isPrecisionRouteMovement()
                || (Baritone.settings().freeLook.value
                        && handler.baritone.getPathingBehavior().isPathing()
                        && handler.baritone.getPathingBehavior().getCurrent() != null
                        && handler.baritone.getLookBehavior().shouldDecoupleMovementFromVisualYaw());
    }
